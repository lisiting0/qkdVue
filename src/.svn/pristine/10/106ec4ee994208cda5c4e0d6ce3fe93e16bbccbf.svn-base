...
export default {
    ...
    if(state.userId){
      ...
      try{
        if(state.scoketConnect=='disconnect'){
          ...
          // Vue.use(VueSocketIO, socketio(state.socketUrl+'?token='+"Bearer " + state.Authorization), store);
		  //console.log("dd")
  ...
  disconnectSocket({commit,state,dispatch}){
    Vue.prototype.$socket&&Vue.prototype.$socket.close();
  },
        ...
        if(message.chatType==1) {//私聊
          ...
          if(message.msgType==0) {//文本消息
            ...
            if(message.groupId==state.currentChatRoomId){
				...
				//console.log("ddddddddddddddddd:"+JSON.stringify(gift))
              commit('CHANGEGIFTOBJ',gift);
			 /*if(gift.giftMsg){
				 util.handleWebGift(message.sourceClientId,type,'','',message.sourceClientId,(message.createDate),'',state.userId,message.msgId,gift.giftId,gift.giftImage,gift.giftName,gift.giftNum)
				commit('CHANGEGETNEWMESSAGE',true)
				dispatch("getConversationList") 
			 }*/
            }
          ...
          }else if(message.msgType==7&&message.msgContent=='smess_sendGift'){//约会送花显示在聊天记录  
            ...
            if(!onlyDetail){
              util.handleWebMessageList(message.groupId,type,icon,name,isZero?0:1,'[送出礼物]',message.createDate,message.msgId,state.userId,message.sourceClientId,activeState,activeType)
            }  
			let giftObj = JSON.parse(message.extras);
			  util.handleWebGift(message.groupId,2,'','',message.sourceClientId,message.createDate,'',message.sourceClientId,message.msgId,giftObj.giftId,giftObj.giftImage,giftObj.giftName,giftObj.giftJiaobi,JSON.stringify(giftObj.text))
			dispatch("getConversationList")
		  }
        }
      }
  ...
  async processingMessage({commit,state,dispatch},message){
    //处理私聊  chatType  1:私聊,2:群聊,3讨论组  
	//msgType 0:文本消息，1:图片消息,2:语音消息,3:视频信息,4:音乐,5:图文,6定位消息，7命令消息
      ...
      if(message.chatType==1){//私聊
        ...
        if(message.msgType==0) {//文本消息
          ...
          if(!db.getUnreadCountByTargetId(message.groupId,state.userId)){
           ...
           db.insertOrUpdateUnreadCountName(message.groupId,type,icon,name,1,'[送出礼物]',message.createDate,message.msgId,state.userId,message.sourceClientId,activeState,activeType);
		  
			//console.log("000000000000000000:"+JSON.stringify(message));
		   db.insertGift(message.groupId,type,extras.userImage,extras.userName,extras.userId,(message.createDate),'',state.userId,message.msgId,extras.giftId,extras.giftImage,extras.giftName,extras.giftJiaobi,JSON.stringify(extras.text))
		   
		  
          if(message.groupId==state.currentChatRoomId){
            let dataChats=db.getConversation(message.groupId,state.userId,100000);
            db.resetCount(message.groupId,state.userId);
			//console.log("ssssssssssssssss:"+JSON.stringify(dataChats));
            commit('CHANGECHATROOMMESSAGELIST',dataChats)
          }
          if(!state.active){//弹出通知消息
            let name = db.getUserName(message.sourceClientId)
  ...
  async getMylocation({state},data){
    if(state.isWeixin){
      try{
        let GPS ={
				...
				success: function (data) { 
					if(data.status==0){
						let bdMapObj={//应用内百度地图模块返回的值
							...
							province: data.result.addressComponent.province,              //字符串类型；省份
							city: data.result.addressComponent.city,                  //字符串类型；城市
							district: data.result.addressComponent.district,              //字符串类型；县区
							streetName: data.result.addressComponent.street,            //字符串类型；街道名
							streetNumber: data.result.addressComponent.street_number,          //字符串类型；街道号
							country:data.result.addressComponent.country,                //字符串类型；国家
							countryCode:data.result.addressComponent.country_code,            //字符串类型；国家代码
							adCode:data.result.addressComponent.adcode,                 //字符串类型；行政区域编码
							businessCircle:data.result.business,         //字符串类型；商圈名称
							sematicDescription:data.result.sematic_description,     //字符串类型；结合当前位置POI的语义化结果描述
							cityCode:data.result.cityCode,               //字符串类型；城市编码
							lon: json.lon,              //数字类型；经度
							lat: json.lat,               //数字类型；纬度
							address: data.result.formatted_address,               //字符串类型；地址信息
							poiList:[{                 //数组类型；经纬度点热点列表
		...
		try{
			data=data||{}
			console.log("data.getCachedata.getCache:"+data.getCache)
			if(data.getCache&&data.callback){
				let myLocation=getStore("myLocation");
				if(myLocation){
					console.log("定位读缓存的");
			...
			let obj= await new Promise((resolve,reject)=>{
				...
				let amapinit=()=>{
					AMap.plugin('AMap.Geolocation',()=>{
						const geolocation = new AMap.Geolocation({
							...
							noGeoLocation:0,
							extensions:"base",//默认为'base',只返回地址信息；设定为'all'的时候将返回周边POI、道路交叉口等信息。
						});
						//geolocation.isSupported()//是否支持浏览器定位，当不支持是getCurrentPosition也会使用尝试进行精确IP定位
						geolocation.getCurrentPosition((status,result)=>{
							console.log("获取地址:"+status,result);
							if(status=="complete"){
								clearTimeout(window.amapTimer);
								let aMapObj={//应用内百度地图模块返回的值
									status: false,              //布尔型；true||false
									province: result.addressComponent&&result.addressComponent.province,              //字符串类型；省份
									city: result.addressComponent&&result.addressComponent.city,                  //字符串类型；城市
									district: result.addressComponent&&result.addressComponent.district,              //字符串类型；县区
									streetName: result.addressComponent&&result.addressComponent.street,            //字符串类型；街道名
									streetNumber: result.addressComponent&&result.addressComponent.streetNumber,          //字符串类型；街道号
									country:result.country,                //字符串类型；国家
									countryCode:'',            //字符串类型；国家代码
									adCode:'',                 //字符串类型；行政区域编码
									businessCircle:'',         //字符串类型；商圈名称
									sematicDescription:'',     //字符串类型；结合当前位置POI的语义化结果描述
									cityCode:'',               //字符串类型；城市编码
									lon: result.position.lng,              //数字类型；经度
									lat: result.position.lat,               //数字类型；纬度
									address: result.formattedAddress,               //字符串类型；地址信息
									poiList:[{                 //数组类型；经纬度点热点列表
									   name: result.addressComponent&&result.addressComponent.street,               //字符串类型；热点名称
									   uid: '',                //字符串类型；热点id
									   address: result.addressComponent&&result.addressComponent.street,            //字符串类型；热点地址
									   city: result.addressComponent&&result.addressComponent.city,               //字符串类型；热点所在城市
									   province: result.addressComponent&&result.addressComponent.province,
									   phone: '',              //字符串类型；热点电话
									   postcode: '',           //字符串类型；热点邮编
									   epoitype: '',           //字符串类型；热点类型，0:普通点 1:公交站 2:公交线路 3:地铁站 4:地铁线路
									   coord: {                //JSON对象；热点坐标信息
										  lat:result.position.lat ,               //数字类型；热点纬度
										  lon:result.position.lng               //数字类型；热点经度
									   }
									}]
								};
								if(aMapObj.country=="中华人民共和国"){
									resolve(aMapObj);
								}else{
									reject("failed"); 
								}
								...
								geolocation.getCityInfo((status,result)=>{
									...
									if(status=="complete"){	
										let url="http://api.map.baidu.com/geocoder/v2/?ak="+state.baiduKey+"&callback=renderReverse&location="+result.center[1]+","+result.center[0]+"&output=json&pois=0"
										$.ajax({ 
											type: "GET", 
											dataType: "jsonp", 
											url: url, 
											success: function (data) { 
												if(data.status==0){
													let bdMapObj={//应用内百度地图模块返回的值
														...
														sematicDescription:data.result.sematic_description,     //字符串类型；结合当前位置POI的语义化结果描述
														cityCode:data.result.cityCode,               //字符串类型；城市编码
														lon: result.center[0],              //数字类型；经度
														lat: result.center[1],               //数字类型；纬度
														address: data.result.formatted_address,               //字符串类型；地址信息
														poiList:[{                 //数组类型；经纬度点热点列表
														   name: data.result.poiRegions[0]&&data.result.poiRegions[0].name||data.result.addressComponent.street,               //字符串类型；热点名称
														   uid: data.result.poiRegions[0]&&data.result.poiRegions[0].uid||123456789,                //字符串类型；热点id
														   ...
														   epoitype: 0,           //字符串类型；热点类型，0:普通点 1:公交站 2:公交线路 3:地铁站 4:地铁线路
														   coord: {                //JSON对象；热点坐标信息
															  lat:result.center[1] ,               //数字类型；热点纬度
															  lon:result.center[0]               //数字类型；热点经度
														   }
														}]
													};
													if(bdMapObj.country=="中国"){
											...
											error: function (XMLHttpRequest, textStatus, errorThrown) { 
												reject(result.center[1]+","+result.center[0]+"地址位置获取失败"); 
											}
										});
										/*let aMapObj={//应用内百度地图模块返回的值
											status: true,              //布尔型；true||false
											province: result.province,              //字符串类型；省份
											city: result.city,                  //字符串类型；城市
											district: '',              //字符串类型；县区
											streetName: '',            //字符串类型；街道名
											streetNumber: '',          //字符串类型；街道号
											country:result.country,                //字符串类型；国家
											countryCode:'',            //字符串类型；国家代码
											adCode:result.adcode,                 //字符串类型；行政区域编码
											businessCircle:'',         //字符串类型；商圈名称
											sematicDescription:'',     //字符串类型；结合当前位置POI的语义化结果描述
											cityCode:result.citycode,               //字符串类型；城市编码
											lon: result.center[0],              //数字类型；经度
											lat: result.center[1],               //数字类型；纬度
											address: '',               //字符串类型；地址信息
											poiList:[{                 //数组类型；经纬度点热点列表
				...
				if(!window.initAmap){
					let script = document.createElement("script");
						...
						//script.src = "http://api.map.baidu.com/api?v=2.0&ak=ccUFvD5ASGQqictfXpLuFGdt&callback=initAmap";
						script.src="https://webapi.amap.com/maps?v=1.4.8&key="+state.AMapApiKey+"&callback=initAmap";
      ...
      try{
		...
		if(obj.province&&obj.poiList&&!obj.poiList.province){
			obj.poiList.province=obj.province;
		}
        return Promise.resolve(obj)
      }catch (e) {
        return Promise.resolve({
  ...
  [SEARCHHISTORY]({ commit }) {
	...
	getCancelToken({commit,state}){//获取axios的CancelToken
		const CancelToken = axios.CancelToken;
		store.commit("CHANGESTORE",{name:"axiosCancelSource",value:CancelToken.source()});
	},
	cancelRequest({commit,state},text){//中断axios的请求
		state.axiosCancelSource.cancel&&state.axiosCancelSource.cancel(text);
	},
	changeHandleBackRouteName({state},{type,routerName}){//配合routerback使用,修改当前监听页面
		if(type=="add"){
