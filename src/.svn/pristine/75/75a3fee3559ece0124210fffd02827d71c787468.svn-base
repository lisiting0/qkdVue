    ...
    <div class="user-div">
      ...
      <div class="user_top" :class="{userTop:userTop}">
        ...
        <div class="user_info" style="position: relative;">
          <router-link :to="{name:'userInfo',query:{id:$store.state.userInfo.id}}" tag="i" class="user_img" :style="'background-image:url('+(getFullPath($store.state.userInfo.headimgAttachmentId)||imgs.headImage)+');'" v-if="$store.state.isLogin"></router-link>
        ...
        <div class="user_menu">
          ...
          <a @click="linkTo('/myOffer')">我的邀约<br/><span class="time">{{closeTime}}</span></a>
        </div>
        <ul class="user_l">
      ...
      <div :to="{name:'share'}" tag="div" class="invite-cls" @click="toShareLink">
        <p><span class="yqhy-icon"></span>邀请好友<i class="iconfont">&#xe702;</i></p>
      </div>
