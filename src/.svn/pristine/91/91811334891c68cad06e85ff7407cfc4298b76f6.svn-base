<template>
  <div>
    <div class="main jy_all_top jy_new_love jy_state_top">
          ...
          <ul class="jy_new_love_list">
            <router-link tag="li" v-for="item,index in showList" :key="index"  :to="{name:'act',query:{id:item.id}}" :class="{jy_new_love_list_oneImg:item.showImg&&item.showImg.length==1}">
              <div class="jy_new_love_list_t">
                ...
                <p><em v-if="!$store.state.isiOSApp">{{item.payType==0?'AA':item.payType==1?'我请客':item.payType==2?'你买单':''}}</em><i class="jy_auth_age" :class="{jy_auth_age_man:item.jiaoyouUser.sex==2}">{{item.jiaoyouUser.sex==1?'&#xe64a;':'&#xe605;'}}{{item.jiaoyouUser.age}}</i></p>
              </div>
              <div v-if="item.showImg&&item.showImg.length==1" class="jy_new_love_list_b">
                ...
                <p class="jy_new_love_list_date">{{item.activityStarttime&&item.activityStarttime.substr(0,10)}}</p>
      ...
      <div class="publishButton" v-if="!$store.state.isBrowser&&!$route.query.nearbydate"  @click.stop="chooseThemes"><i class="iconfont" >&#xe61f;</i></div>
    </div>
...
import setSort from '../../affinity/setSort'
