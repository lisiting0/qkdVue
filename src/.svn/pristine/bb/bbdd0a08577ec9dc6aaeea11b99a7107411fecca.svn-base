...
//字典(返回的是对象)
server. apiDictObject=(data,config)=>{
  return	http.get("/api/apiDict/getRemarkValueToKey",data,config);
}
//字典
...
//娇缘活动单人聚会选人
server. choiceUser=(data,config)=>{
  return	http.post("/api/dating/choiceUser",data,config);
}
//最美娇友总榜
...
//取消发布活动
server. unDating=(data,config)=>{
  return	http.post("/api/dating/unDating",data,config);
}
//活动详情
...
//活动点赞接口api/dating/pointLike
server. pointLike=(data,config)=>{
  return http.post("/api/dating/pointLike",data,config);
}
//参加活动接口
...
//我的待赴约的活动（我的邀约：待赴约）
server.getWaitingDating=(data,config)=>{
  return	http.get("/api/dating/getWaitingDating",data,config);
}
//获取所有的保证金等级
...
//获取合伙人列表
server.getOrdAgent=(data,config)=>{
  return http.get("/api/ordAgent/list",data,config);
}
//任务宝领取任务
server.receiveTasks=(data,config)=>{
  return http.get("/api/invitation/receiveTasks",data,config);
}
//任务宝任务详情
server.getTask=(data,config)=>{
  return http.get("/api/invitation/getTask",data,config);
}
//任务完成填写收货地址的接口
server.updateAddress=(data,config)=>{
  return http.get("/api/invitation/updateAddress",data,config);
}
//我邀请的微信关注用户
server.myInvitedWx=(data,config)=>{
  return http.get("/api/invitation/myInvited",data,config);
}
//生成二维码
server.referralCode=(data,config)=>{
  return http.get("/api/invitation/referralCode",data,config);
}
//生成二维码
server.getOpenId=(data,config)=>{
  return http.get("/api/invitation/getOpenId",data,config);
}
//静态活动页面
server.getActStatic=(data,config)=>{
	return http.get("/wx/staticAct/actStatic.data.js",data,config);
}
//静态帮助页面
server.getHelpStatic=(data,config)=>{
	return http.get("/wx/staticAct/help.js",data,config);
}
//产生访问足迹
server.createVisitRecord=(data,config)=>{
  return http.get("/api/user/createVisitRecord",data,config);
}
//获取流水记录类型
server.findParentList=(data,config)=>{
  return http.get("/api/jiaoyouPayTypeInfo/findParentList",data,config);
}

//发布曝光台
server.lightPublish=(data,config)=>{
  return http.postForm("/api/light/publish",data,config);
}
//查看曝光台
server.lightGetList=(data,config)=>{
  return http.get("/api/light/getList",data,config);
}
//查看我的曝光台
server.lightGetMyList=(data,config)=>{
  return http.get("/api/light/getMyList",data,config);
}
//发表评论
server.commentLightHouseCreate=(data,config)=>{
  return http.postForm("/api/commentLightHouse/create",data,config);
}
//评论列表
server.commentLightHouseList=(data,config)=>{
  return http.get("/api/commentLightHouse/list",data,config);
}
//删除评论
server.commentLightHouseDelete=(id,data,config)=>{
  return http.delete("/api/commentLightHouse/delete/"+id,data,config);
}
//退出登录
server.loginOut=(data,config)=>{
  return http.post("/api/user/loginOut",data,config);
}
//评论列表
server.getUnderWayTask=(data,config)=>{
  return http.get("/api/task/getUnderWayTask",data,config);
}
