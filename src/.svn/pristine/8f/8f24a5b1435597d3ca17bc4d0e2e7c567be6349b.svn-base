      ...
      <transition enter-active-class="slideInDown" leave-active-class="slideOutDown">
        <div class="filterPopover" v-show="showFilter" @click.stop>
          <div>
            <p>地区</p>
            <div>
              <span @click="selectLocation=''" :class="{active:locationCopy==''}">不限</span>
              <span @click="showPlacePicker" :class="{active:locationCopy==1}" style="width: 50%;">{{place===''?'选择地点':place}}</span>
            </div>
...
<script>
  ...
  export default {
        ...
        methods: {
          ...
          filterMakesure() {
            this.$emit('filterMakesure', this.locationCopy,this.selectArea,this.selPlace,this.datingTime,this.startDate,this.endDate,this.specialTypeVal)
          },
        }
    }
</script>
