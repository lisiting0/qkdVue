<template>
  <div class="main jy_all_top jy_state_top ">
    ...
    <myScroller style="background-color: #FFF;" :style="$store.state.isBrowser?'padding:3.05rem 0 0;':'padding:3.95rem 0 0;'" :infinite="infinite" :refresh="refresh" :loadRefresh="loadRefresh" :loadInfinite="loadInfinite" ref="myScroller" :showbg="showbg">
          ...
          <li @click="linkTo('userInfo',item.jiaoyouUser.id,item.id,item.jiaoyouUser.silentState)" v-for="(item,index) in listArr" :key="item.id">
            ...
            <template v-if="datingDetail.jiaoyouUser&&(datingDetail.jiaoyouUser.id == $store.state.userId)&&datingDetail.activityMethod==7">
              <div class="button" v-if="datingDetail.state==5">
                ...
                <p v-else style="background-color: #f2f2f2;color:#CCC">和Ta约会</p>
              </div>
              <div class="button" v-else>
                <p @click.stop="makesure(item.jiaoyouUser.id)">和Ta约会</p>
              </div>
            </template>
          </li>
        </ul>
