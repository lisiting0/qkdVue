			...
			<dd>
				<ul class="jy_earn_tlist">
					<li v-for="item in objv.arr" :key="item.id">
						<i class="jy_earn_timg" :bgImg="getFullPath(item.icon)"></i>
            <div>
...
<script>
  ...
  import shop from '@other/shop.vue';
    ...
    methods :{
		getFullPath (path) {
			return this.$utils.getFullPath(path+"!w240")
		 },
		setBgImg(){
      ...
      toLink(urlLink){
		    ...
		    let urlArr = urlLink.split("-");
		    let url = urlArr[0]
		    let url1 = urlArr[1]
        if(url1&&this.$store.state.isApp){
          this.afterToLink(url1);
          return
        }
        this.afterToLink(url);
      }
    }
  }
</script>
