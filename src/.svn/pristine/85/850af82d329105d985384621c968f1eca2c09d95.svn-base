...
<div class="jy_act_scroll jy_act_blindPrize jy_act_blindNew">
		...
		<div class="page_1 page1">
			<div class="horizontalScroll_1">
				<div class="page_2 s1" :class="{page_hide:showFull}">
					<div class="header">
						<div class="top">
							<a class="left" @click.stop="goBack"><i class="iconfont">&#xe613;</i></a><div class="right">
								...
								<a @click.shop="userRefresh" v-if="isManage||isAdmin"><i class="iconfont">&#xe810;</i></a>
								...
								<a v-if="!isManage&&!isAdmin" @click.stop="showFull=!showFull"><i class="iconfont">{{showFull?'&#xe690;':'&#xe607;'}}</i></a>
							</div><p class="opa_hide">{{owner&&owner.datingTitle}}</p>
							<!--<i class="j_bottom" v-if="!isManage&&!isAdmin" @click.stop="showFull=!showFull">{{showFull?'&#xe690;':'&#xe607;'}}</i>-->
						</div>
					</div>
					<div class="jy_act_top" v-if="owner&&chatRoomInfo">
						<template v-if="isManage||isAdmin">
						...
						<template v-if="chatRoomInfo">
							...
							<p class="jy_act_bl_tip" v-if="isManage||isAdmin">报名人数：{{baomingUser&&baomingUser.length||owner.enrollCount||0}}人,签到人数:{{chatRoomInfo.baoMingCount||0}},男:{{chatRoomInfo.lookOnCount}},女:{{chatRoomInfo.baoMingCount-chatRoomInfo.lookOnCount}},已匹配:{{currMatchUsers.length}}对,剩余:(男:{{chatRoomInfo.lookOnCount-currMatchUsers.length}},女:{{chatRoomInfo.baoMingCount-chatRoomInfo.lookOnCount-currMatchUsers.length}});
					...
					<div class="jy_act_mid" @transitionend="transitionEnd" :class="{jy_act_mid_manage:isManage||isAdmin||phListType!=6,jy_act_mid_dia:phListType==5}">
						<div class="jy_act_mid_con">
							<div class="jy_act_bl_scroll " :class="{on:phListType==6||phListType==7}">
								<div class="jy_bd_menu" :class="{cur:isManage||isAdmin}">
									<i :class="{cur:phListType==5,on:phListCur.c5}" @click.stop="phListType=5">群聊 &#xe650;</i>
									<i v-if="isManage||isAdmin" :class="{cur:phListType==6,on:phListCur.c6}" @click.stop="phListType=6">嘉宾 &#xe6db;</i>
									...
									<i v-if="isManage||isAdmin" :class="{cur:phListType==7,on:phListCur.c7}" @click.stop="phListType=7">抽奖 &#xe684;</i>
								</div>
								...
								<ul v-else-if="phListType==3" key="list3" class="jy_bd_menu_result"><!-- 匹配结果 -->
									<li v-for="item,index in allMatchUsers" :data-num="index+1">
										<div class="jy_bd_menu_c" :class="{jy_bd_menu_c_blur:!isManage&&!isAdmin&&item[0].userId!=$store.state.userInfo.id&&item[1].userId!=$store.state.userInfo.id&&item[0].status==0&&item[1].status==0}">
											...
											<em class="jy_bd_menu_result_type" :class="{cur2:item[0].number==2,cur3:item[0].number==3}" v-if="(isManage||isAdmin)&&(item[0].number==1||item[0].number==2||item[0].number==3||item[0].number==5||item[1].number==1||item[1].number==2||item[1].number==3||item[1].number==5)">{{item[0].number==1||item[1].number==1?'双向配':item[0].number==2||item[1].number==2?'男求配':item[0].number==3||item[1].number==3?'女求配':item[0].number==5||item[1].number==5?'抽对象':''}}</em>
										</div>
										<i v-if="item[0].status==0&&(isManage||isAdmin)" class="jy_bd_menu_btn" @click.stop="publishMatch(item)">公布</i>
										<i v-if="item[0].status==0&&!isManage&&!isAdmin" class="jy_bd_menu_btn off">未公布</i>
								...
								<template  v-else-if="phListType==6"><!-- 人员列表 -->
									...
									<ul class="jy_bd_user" key="list6" :class="'userStyle_'+(userStyle%3)">
										<!-- 推女方/推男方 -->
										<template v-if="chatRoomInfo&&!myMathch&&chatRoomInfo.status==1&&!isManage&&!isAdmin&&myLove.length>0&&((chatRoomInfo.number==2&&$store.state.userInfo.sex==1)||(chatRoomInfo.number==3&&$store.state.userInfo.sex==2))">
										...
										<template v-else-if="searchList.length>0||searchKey!=''">
											<template v-if="searchList.length>0">
												<li v-for="item,index in searchList" v-if="$store.state.userInfo.id!=item.jiaoyouUser.id" @click.stop="$store.state.userInfo.sex!=item.candidateSex||isManage||isAdmin?getUserInfo(item.jiaoyouUser.id):''">
													<i class="jy_bd_user_img" :style="'background-image:url('+$utils.getFullPathMin(item.jiaoyouUser.headimgAttachmentId)+')'">
														<em class="jy_bd_choose" v-if="!isActEnd&&!isAgree&&!isAgreeMe&&!myMathch&&!isManage&&!isAdmin&&isSign&&item.ext&&item.ext.extInt&&item.candidateSex!=$store.state.userInfo.sex&&!checkAgree(item.jiaoyouUser.id)" :class="{cur:item.jiaoyouUser.isLike==3}" @click.stop="saveSingleLike(getBaomingUserIndex(item.jiaoyouUser.id))"></em>
														<em v-if="!isManage&&!isAdmin&&item.candidateSex!=$store.state.userInfo.sex" class="jy_bd_love" :class="{cur:item.jiaoyouUser.isGood}" @click.stop="myFeel(getBaomingUserIndex(item.jiaoyouUser.id))"></em>
														<em v-if="item.candidateSex!=$store.state.userInfo.sex||isManage||isAdmin" class="jy_bd_com" :class="{cur:msgLength[item.jiaoyouUser.id]}" :data-num="msgLength[item.jiaoyouUser.id]" @click.stop="showMsg=true,msgUser={name:item.jiaoyouUser.aliasName,userId:item.jiaoyouUser.id,img:item.jiaoyouUser.headimgAttachmentId}"></em>
														<span>{{item.ext&&item.ext.extInt?getSex(item.candidateSex)+item.ext.extInt+"号":"待签到"}}</span>
													</i>
													<p class="jy_bl_select_ico"><i class="jy_auth_age" :class="{jy_auth_age_man:item.candidateSex==2}">{{item.candidateSex==1?'&#xe64a;':'&#xe605;'}}{{item.jiaoyouUser.age}}</i>{{item.jiaoyouUser.aliasName}}</p>
												</li>
										...
										<!-- 正常列表 -->
										<template v-else>
											<li v-for="item,index in showBaomingUser" v-if="$store.state.userInfo.id!=item.jiaoyouUser.id" @click.stop="$store.state.userInfo.sex!=item.candidateSex||isManage||isAdmin?getUserInfo(item.jiaoyouUser.id):''">
												<i class="jy_bd_user_img" :style="'background-image:url('+$utils.getFullPathMin(item.jiaoyouUser.headimgAttachmentId)+')'">
													<em class="jy_bd_choose" v-if="!isActEnd&&!isAgree&&!isAgreeMe&&!myMathch&&!isManage&&!isAdmin&&isSign&&item.ext&&item.ext.extInt&&item.candidateSex!=$store.state.userInfo.sex&&!checkAgree(item.jiaoyouUser.id)" :class="{cur:item.jiaoyouUser.isLike==3}" @click.stop="saveSingleLike(getBaomingUserIndex(item.jiaoyouUser.id))"></em>
													<em v-if="!isManage&&!isAdmin&&item.candidateSex!=$store.state.userInfo.sex" class="jy_bd_love" :class="{cur:item.jiaoyouUser.isGood}" @click.stop="myFeel(getBaomingUserIndex(item.jiaoyouUser.id))"></em>
													<em v-if="item.candidateSex!=$store.state.userInfo.sex||isManage||isAdmin" class="jy_bd_com" :class="{cur:msgLength[item.jiaoyouUser.id]}" :data-num="msgLength[item.jiaoyouUser.id]" @click.stop="showMsg=true,msgUser={name:item.jiaoyouUser.aliasName,userId:item.jiaoyouUser.id,img:item.jiaoyouUser.headimgAttachmentId}"></em>
					...
					<div class="jy_act_bottom2" v-if="!isManage&&!isAdmin&&phListType==6">
	...
	<div v-transfer-dom>
		<popup v-model="showApplyWin" height="100%" :hide-on-blur=false position="bottom" :popup-style="{zIndex:601}" :should-scroll-top-on-show="true">
	...
	<div v-transfer-dom>
		<popup v-model="showGoodWin" height="100%" :hide-on-blur=false position="bottom" :popup-style="{zIndex:601}" :should-scroll-top-on-show="true">
		<div class="top_userInfo" v-if="showGood">
			<favor :datingId="datingId" @openWin="openGoodWin" @hiddenInfo="hiddenGoodInfo" @getUserInfo="getUserInfo"></favor>
		</div>
		</popup>
	</div>
	<div v-transfer-dom>
		<popup v-model="showVisitWin" height="100%" :hide-on-blur=false position="bottom" :popup-style="{zIndex:601}" :should-scroll-top-on-show="true">
	...
	<div v-transfer-dom>
		<popup v-model="showFocusWin" height="100%" :hide-on-blur=false position="bottom" :popup-style="{zIndex:601}" :should-scroll-top-on-show="true">
	...
	<div v-transfer-dom>
		<popup v-model="showDiaMsg" height="100%" :hide-on-blur=false position="bottom" :popup-style="{zIndex:601}" :should-scroll-top-on-show="true">
  ...
  <div v-transfer-dom>
    <popup v-model="showPrizeDraw" height="100%" :hide-on-blur=false position="bottom" :popup-style="{zIndex:601}" :should-scroll-top-on-show="true">
	...
	<div v-transfer-dom>
		<x-dialog v-model="showQcode" :hide-on-blur=true>
			<div class="jy_act_bl_qcode" v-if="isManage||isAdmin">
				<p>扫描以下二维码签到</p>
	...
	<div class="jy_dia" v-if="showBroad&&(msgUser||isManage||isAdmin)">
		<div class="jy_dia_c">
			...
			<ul v-if="isManage||isAdmin" class="jy_act_lt_manage_msg">
			...
			<div class="jy_dia_bottom">
				...
				<i v-if="isManage||isAdmin" @click.stop="sendBroad('')">发布</i>
			</div>
		</div>
	</div>
...
<script>
      ...
      doMessage(newV,oldV){
		...
		listenChatRoomBroad(broad){//收到主持人广播信息
			...
			if(broad.type=='broad'&&broad.BroadContent){
				...
				if(t.baomingUser[index].candidateSex==2){
					t.chatRoomInfo.lookOnCount++;
				}
				t.chatRoomInfo.baoMingCount++;
			}
		},
		listenChatRoomMessageList(newList){//聊天消息列表，用于展示
			const t=this;
			console.log('相亲聊天信息'+t.$store.state.userInfo.aliasName+'：'+JSON.stringify(newList))
			if(!newList||newList.length<0||(t.chatList&&t.chatList.length==newList.length)){
				return false;
			}
		...
		currMatchUsers(val){//已匹配的人员列表
			const t=this;
			if(!t.isManage&&!t.isAdmin){//非主持,找出已经我已经匹配的.
		...
		async chatRoomInfoMsg(val){//活动信息更新(app使用)
			...
			if(!t.isSign&&!t.isManage&&!t.isAdmin){//没有签到
				if(val.status2){
    ...
    async mounted(){
		...
		//t.audioAutoPlay();
		//if(!t.isManage&&!t.isAdmin){//非主持人才能滑屏
    ...
    methods :{
		...
		**/
		computeWeight(obj){//计算权重
			...
			let isLike=150,//中意优先
				...
				noSign=-1000;//未签到
			let aWeight=0;
			if(!t.isManage&&!t.isAdmin){//主持人不需要考虑中意等
		...
		async datingTaskRewardRanking(){//人气排行
			...
			try{
				...
				const result = await t.$server.datingTaskRewardRanking({
					datingId:t.datingId,
					cacheTimeout:10,//(秒)
					rows:t.isManage||t.isAdmin?10000:20,
				});
				t.isloading=false;
      ...
      beginRecord(){
		...
		getUserInfo(userId){
			...
			if(t.isManage||t.isAdmin||(t.$store.state.userInfo.idStatus==1&&t.$store.state.userInfo.silentState==0)){
				t.showInfo=true;
				t.userId=userId;
			}else{
				if(t.$store.state.userInfo.silentState!=0&&t.$store.state.userInfo.idStatus!=1){
					t.$vux.confirm.show({
						...
						theme: 'android',
						hideOnBlur:true,
						content: '只有关闭"不允许别人看我"以及通过"实名认证"之后，才能查看其他嘉宾的详细信息。',
						confirmText: '去实名认证',
						cancelText:'去关闭',
						onCancel () {
					...
					t.$vux.confirm.show({
						...
						theme: 'android',
						hideOnBlur:true,
						content: '只有关闭"不允许别人看我"之后，才能查看其他嘉宾的详细信息。',
						confirmText: '去关闭',
						onCancel () {
					...
					t.$vux.confirm.show({
						...
						theme: 'android',
						hideOnBlur:true,
						content: '只有通过"实名认证"之后，才能查看其他嘉宾的详细信息。',
						confirmText: '去实名认证',
						onCancel () {
		...
		async getSingle(){//获取活动详情
			...
			try{
				...
				if(t.owner.adminList&&t.owner.adminList.length>0){
					let isAdmin=t.owner.adminList.filter((val)=>{
				...
				if(t.owner.isCandidate==0&&!t.isManage&&!t.isAdmin){//没报名(不是群主)
