<template>
  <div class="main jy_state_pop jy_nearby jy_nearby_home">
    ...
    <div class="myscoll" v-else>
	  <myScroller :style="$store.state.isBrowser?'padding:1.4rem 0 1.62rem;':'padding:2.3rem 0 1.62rem;'" :infinite="infinite" :refresh="refresh" :noDataText="noDataText" :loadRefresh="loadRefresh" :loadInfinite="loadInfinite" ref="myScroller" :showbg="showbg">
      ...
      async getDataList(loc,layoutType,search,PullingDown,flag) {
        ...
        try{
		  ...
		  t.layoutType = layoutType;
			...
			t.isRefresh = false;
			console.log("首页异常"+JSON.stringify(e));
        }
		if(!flag){
			t.page++;
			t.isRead = false;
