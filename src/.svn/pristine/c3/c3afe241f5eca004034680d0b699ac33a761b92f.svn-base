		...
		shareWebpage(Vscene){//分享到微信
			...
			t.wx.isInstalled(function(ret, err) {
				if (ret.installed) {
					...
					let obj={
						title : t.shareTitle||t.$route.meta.shareTitle||t.$store.state.custom_config.defaultShareTile||'乾坤岛',
						description : t.shareDesc||t.$route.meta.shareDesc||t.$store.state.custom_config.defaultShareContent||'一个安全、纯粹、高效的婚恋交友平台',
