...
export default {
 ...
 /* [CHANGEISTOCITY] (state, flag) {
  ...
  [CHANGEACCESSTOKEN] (state, data) {
	...
	state.keepAlive=[];//登录后要清除一下页面缓存.(首页/性别)
	state.cachelist = {}//清除页面切换缓存
	state.localcache={}//清除缓存
  },
  clearAllCache(state){
		state.keepAlive=[];//登录后要清除一下页面缓存.(首页/性别)
		state.cachelist = {}//清除页面切换缓存
  ...
  ['CHANGEIMAGEURL'](state,data){
	...
	state.baseFrontEndURL=data.baseFrontEndURL||state.baseFrontEndURL;
	 if((state.isiOSApp||state.isApp)&&!state.debug){
	 	state.baseURL=data.appBaseURL||state.baseURL;
	 	state.socketUrl=data.appleSocketUrl||state.socketUrl;
		state.imageUrl = data.appBaseURL?data.appBaseURL+"/userfiles/":data.jiaoyouAttachmentPath||state.imageUrl;
		state.imageUploadUrl = data.appBaseURL||data.jiaoyouImgBaseUrl||state.baseURL;
		state.baseShopMobileURL=data.appBaseURL?data.appBaseURL+"/mobile/":data.baseShopMobileURL||state.baseShopMobileURL;
		state.baseFrontEndURL=data.appBaseURL?data.appBaseURL+"/wx/":data.baseFrontEndURL||state.baseFrontEndURL;
		state.phpURL = data.appBaseURL||data.shopBaseUrl||sstate.baseURL;
	 }
	 state.isGetConfigUrl=true;
  },
  //修改socket连接状态
  ...
  ['CHANGEHANDLEROUTE'](state,handleRoute){
	...
	console.log("handleRoute改变:"+handleRoute)
	/*if(handleRoute){
  ...
  ['UPDATAPUBLISHACTIVITY'](state,publishActivity){
  ...
  [LOGINOUT] (state) {//登出
    ...
    //处理消息相关
    state.unreadCount=0;
    state.rongConversationList=[];
    state.webActiveMessageList=[];
    state.webSystemMessageList=[];
    state.webSystemMessageDetailsList=[];
    state.chcheMessageArr=[];
    state.doMessage=true;
    state.webMessageStorage={
      unreadCount:0,//未读消息数
    ...
    state.getNewMessage=false;
    state.getNewNotice=false;
  },
 ...
 /* [SEARCHHISTORY] (state, {data}) {//获取浏览记录
  ...
  [ADDLOCALCACHE] (state, {url, name, data,timeout}) {//缓存
    ...
    //delete name['Access-Token']
	//let saveCache=name['saveCache'];
	//delete name['saveCache']
    const strName = JSON.stringify(name)
   ...
   // console.log(url, strName)
    let storeData={data,creatDate:new Date().getTime(),timeout}
        //if(((name.pageNo&&name.pageNo==1)||!name.pageNo)){//只保存第一页数据跟没页码的数据
        //if(url=="/api/dating/searchDatingList"||url=="/api/user/nearUser"||url=="/api/blindDating/searchDatingList"){//只保存固定接口的数据到本地
		if(name['saveCache']&&((name.page&&name.page==1)||!name.page)){//只保存第一页数据跟没页码的数据
            state.localcacheStore.push({name:url+strName});
            setTimeout(()=>{
                state.localcacheStore.shift();
                setStore(url+strName,storeData);//保存到本地
				console.log("保存缓存到本地:"+(url+strName))
  ...
  [CLEARLOCALCACHE] (state) {
    ...
    for (let i = localStorage.length; i--;) {
      if (localStorage.key(i) != 'loginData' && localStorage.key(i) != 'userId' && localStorage.key(i) != 'userInfo' && localStorage.key(i).indexOf('myLocation') == -1 && localStorage.key(i) != 'Authorization') {
        removeStore(localStorage.key(i))
      }
    }
	state.keepAlive=[];//keepAlive缓存
