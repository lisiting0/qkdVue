...
db. resetCount=function(targetId,ownerId){
  if(store.state.isBrowser){
    ...
    let arr2 = obj.rongConversationList.map(v=>{
      ...
      webcount = webcount+parseInt(v.count||0);
      return v;
    })
...
//保存礼物接口
db. insertGift=function(targetId,type,userIcon,userName,userId,time,remark,ownerId,messageId,giftId,giftImage,giftName,giftJiaobi,text){
  let data = db.insertSqlSync('CONVERSATION', ['TARGET_ID','TYPE','USER_ICON','USER_NAME','USER_ID','TIME','REMARK','OWNER_ID','MESSAGE_ID','GIFT_ID','GIFT_IMAGE','GIFT_NAME','GIFT_JIAOBI','TEXT'], [targetId,type,userIcon,userName,userId,Math.floor(time/1000),remark,ownerId,messageId,giftId,giftImage,giftName,giftJiaobi,text]);
  return data;
}
