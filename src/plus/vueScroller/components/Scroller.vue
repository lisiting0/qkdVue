    ...
    <div class="_v-content" :id="contentId">
      ...
      <div v-if="showInfiniteLayer" class="loading-layer">
        ...
        <div class="no-data-text"
          :class="{'active': (!showLoading && loadingState == 2)||keepText}" :style="{color: loadingLayerColor}" 
          v-text="noDataText">
        </div>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    mounted () {
	  ...
	  //console.log(this.container,this.containerId)
     ...
     // let render = getContentRender(this.content)
      ...
      this.scroller = new Scroller(function(left, top, zoom){
			this.options.bouncing=t.bouncing;
			let render=getContentRender(t.content);
			render(left, top, zoom);
			t.onTouchMove&&t.onTouchMove(left, top, zoom,t.container.clientHeight);
	  }, {
        scrollingX: false,
