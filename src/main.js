...
async function initApp(){
	...
	window.api.requestPermission({
		list:['camera','microphone','photos','location','locationAlways','storage','notification'],
		code:1
	}, function(ret, err){
		console.log("app权限:"+JSON.stringify(ret));
	});
...
let initTimeout;
try{
	server.getBaseConfig({
		readonly:true
	}).then((res)=>{//读配置
		if(res.data.data){
			store.commit("CHANGEIMAGEURL",res.data.data);
			try{
				server.apiDictObject({type:"custom_config",readonly:true}).then((res)=>{//读字典
					if(res.data.data){
