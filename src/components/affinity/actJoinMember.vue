          ...
          <li @click="linkTo('userInfo',item.jiaoyouUser.id,item.id,item.jiaoyouUser.silentState)" v-for="(item,index) in listArr" :key="item.id">
            <a :style="'background-image:url('+getFullPath(item.jiaoyouUser.headimgAttachmentId)+');'">
              ...
              <div class="bottom_info">
                <p><span>{{item.jiaoyouUser.aliasName}}</span><i class="jy_auth_age" :class="{jy_auth_age_man:item.candidateSex==2}">{{item.candidateSex==1?'&#xe64a;':'&#xe605;'}}{{item.jiaoyouUser.age}}</i><span v-if="$store.state.userInfo.identity!=1&&$store.state.userInfo.sex!=item.candidateSex&&item.jiaoyouUser.matchAgree">{{item.jiaoyouUser.matchAgree}}%</span></p>
                <p>{{item.jiaoyouUser.userExt&&item.jiaoyouUser.userExt.height}}cm | {{item.jiaoyouUser.userExt&&educationTextArr[item.jiaoyouUser.userExt.education]}}<span v-if="$store.state.userInfo.identity!=1&&$store.state.userInfo.sex!=item.candidateSex&&item.jiaoyouUser.matchAgree">匹配度</span></p>
              </div>
            </a>
          </li>
        </ul>
