	...
	<div class="main jy_all_top jy_actInfo jy_state_top">
		...
		<div class="jy_block_t">
			<ul class="jy_aInfo_t">
				<li v-if="owner">
					...
					<template v-else>
						<h4>一起{{owner.datingThemes==1?'去旅行':owner.datingThemes==2?'吃饭':owner.datingThemes==3?'看电影':owner.datingThemes==4?'唱歌':owner.datingThemes==5?'运动':''}}·{{owner.areaName}}</h4>
		...
		<div class="jy_block_t">
			...
			<ul class="jy_aInfo_line">
				...
				<router-link v-if="isBlindDate&&owner&&owner.jiaoyouUser.id==$store.state.userInfo.id" tag="li" :to="{name:'adminManage',query:{id:$route.query.id}}">群管理员</router-link>
				<!--<li>分享活动</li>-->
			</ul>
		</div>
