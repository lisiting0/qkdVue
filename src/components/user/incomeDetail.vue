<template>
  <div class="main">
    ...
    <div class="header">
      <div class="top">
        <a class="left" @click="goback"><i class="iconfont">&#xe613;</i></a>{{username}}
      </div>
    </div>
    ...
    <myScroller :style="$store.state.isBrowser?'padding:6.28rem 0 0;':'padding:6.99rem 0 0;'" :infinite="infinite" :refresh="refresh" :loadRefresh="loadRefresh" :loadInfinite="loadInfinite" ref="myScroller" :showbg="showbg">
      <div class="myscoll">
        <div class="content">
...
<script>
  ...
  export default {
    ...
    watch:{
      ...
      '$route' (to, from) {
        ...
        if(to.name==from.name){
          ...
          t.username = t.$route.query.name;
          t.pageNo = 1;
          t.listArr = [];
          t.consumption = {};
          t.accumulatedConsumptionintQkb = 0;//累积娇币消费
          t.accumulatedIncomeQkb = 0;//累积娇币收入
          t.accumulatedCashConsumption = 0;//累积的现金消费
          t.accumulatedCashIncome = 0;//累计的现金收入
          t.getList();
        }
      }
    },
    ...
    methods: {
      ...
      toLink(link, id, name,silentState) {
        let query = null
        if(id){
          query = {
            id: id,
      ...
      async getList(flag) {
        ...
        try {
          ...
          if (!_this.listArr || _this.isRefresh || flag) {
            ...
            _this.consumption = {};
            _this.accumulatedConsumptionintQkb = 0;//累积娇币消费
            _this.accumulatedIncomeQkb = 0;//累积娇币收入
            _this.accumulatedCashConsumption = 0;//累积的现金消费
            _this.accumulatedCashIncome = 0;//累计的现金收入
            _this.listArr = [];
          }
