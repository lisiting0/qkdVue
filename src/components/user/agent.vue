<template>
  <div class="main">
    ...
    <div v-if="showLink==1" class="my_agent">
    ...
    <div v-else-if="showLink==2" class="my_agent">
      <p>您是平台合伙人,当前级别为{{agentLevelName}}</p>
    </div>
    <div class="agent_top " :class="{cur:!showLink}">
      <router-link tag="div" :to="{name:'bill',query:{type:1}}" class="top_con">
      ...
      <router-link tag="div" :to="{name:'bill'}" class="bottom_con">
    ...
    <myScroller :style="$store.state.isBrowser?(showLink?'padding:7rem 0 0;':'padding:5.8rem 0 0;'):(showLink?'padding:8.15rem 0 0;':'padding:6.95rem 0 0;')" :infinite="infinite" :refresh="refresh" :loadRefresh="loadRefresh" :loadInfinite="loadInfinite" ref="myScroller" :showbg="showbg">
      <div class="myscoll">
        <div class="content">
            ...
            <template v-if="listArr.length>0">
              <div v-for="(item,index) in listArr" :key="item.id" @click="toLink('incomeDetail',item.id,item.aliasName)">
                ...
                <div class="userinfo_center">
                  <p v-if="item.agentLevelName">{{item.aliasName}}&nbsp&nbsp({{item.agentLevelName}})</p>
                  <p v-else>{{item.aliasName}}</p>
...
<script>
  ...
  export default {
    ...
    computed: {
      showLink(){
        if(!this.$store.state.userInfo.ordAgent && this.showLinkData){
          return 1;
        }
        if(this.$store.state.userInfo.ordAgent && this.showLinkData){
          return 2;
    ...
    methods: {
      ...
      async getList(flag) {
        ...
        try {
          ...
          _this.agentLevelName=result.data.agentLevelName;
...
.jy_isBrowser{
  ...
  .main {
    ...
    .agent_top {
     position:fixed;z-index:1;left:0;top:3.35rem;width:100%;box-sizing:border-box;-webkit-box-sizing:border-box; background-color: #ffffff;
