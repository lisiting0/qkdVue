<template>
  <div class="main">
    ...
    <myScroller :style="$store.state.isBrowser?'padding:3.13rem 0 0;':'padding:4.03rem 0 0;'" :infinite="infinite" :refresh="refresh" :loadRefresh="loadRefresh" :loadInfinite="loadInfinite" ref="myScroller" :showbg="showbg">
      <div class="myscoll">
        <div class="content">
          <div v-for="(item) in list" :key="item.id">
            <router-link replace :to="{name:'userInfo',query:{id:item.id}}" tag="div"  :style="{'background-image':'url('+$utils.getFullPath(item.headimgAttachmentId)+')'}"></router-link>
