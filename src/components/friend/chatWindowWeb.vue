  ...
  <div class="main">
    ...
    <div class="header">
      <div class="top">
        ...
        <a @click="handle" class="right" v-if="!$store.state.isBrowser"><i class="iconfont">&#xe679;</i></a>{{title}}
      </div>
    </div>
    <div class="myscoll" v-pan="pan" ref="scroll" @scroll="scroll($event)"
         :style="{'margin-top':marginTop+'px','max-height':myScrollerHeight+'px'}">
        ...
        <div class="message-cls">
          ...
          <div class="left" v-if="$store.state.userId!=item.userId">
            ...
            <i class="backImageCover"
               :style="'background-image:url('+$utils.getFullPath(item.bicon)+');width:1.18rem;height:1.18rem;border-radius: 5px;'"></i>
            <div>
              <!--<p>{{item.bname}}</p>-->
              <div :class="{giftDiv:item.giftId}" @click="setActiveItem(item)">
                ...
                <template v-else-if="item.location">
                  <div class="location" @click="openMap(item.lon,item.lat,item.location)"
                       @touchstart="touchstart($event)" @touchend="touchend($event)">
                    <div
                      style="padding:6px 3px 0 6px;font-size: 0.45rem;overflow: hidden;text-overflow:ellipsis;white-space: nowrap;">
                      {{item.location}}
                    </div>
                    <div
                      style="padding:0 3px 3px 6px;color:#979797;font-size: 0.36rem;overflow: hidden;text-overflow:ellipsis;white-space: nowrap;">
                      {{item.address}}
                    </div>
                ...
                <template v-else-if="item.img">
                  <img @click.stop="showImage(item.imgIndex,$event)" @load="imgOnload" :src="item.img"
                       :onerror="errorImg">
                </template>
    ...
    <transition enter-active-class="slideInUp" leave-active-class="slideOutUp">
      <gift v-show="showGift" @sendGift="sendGift" :name="title" :giftUserId="targetId" @close="sendGift"
            :list="giftList" :showShop="true"></gift>
    </transition>
    <div v-transfer-dom>
      <popup v-model="showMapPopup" height="100%" :hide-on-blur="false" position="bottom" :popup-style="{zIndex:596}"
    ...
    computed: {
      ...
      chats: function () {
        ...
        return cloneObj.map(item => {
          ...
          let content = item.message.replace(/[<>]/g, function (c) {
            return {'<': '&lt;', '>': '&gt;', '&': '&amp;', '"': '&quot;'}[c];
          });
          ...
          &.right {
            ...
            & > div {
              ...
              & > div {
                ...
                .arrow em {
                  border-color: transparent transparent transparent #81ca5d;
                }
                .arrow span {
