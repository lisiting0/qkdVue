            ...
            <div>
              ...
              <div :class="{giftDiv:item.giftId}" @click="setActiveItem(item)">
                ...
                <template v-else-if="item.location">
                  <div class="location" @click="$common.openMap(item.lon,item.lat,item.location)" @touchstart="touchstart($event)" @touchend="touchend($event)">
...
import routerBack from '@/plus/routerBack.js';
  routerBack.init("chatWindow",{
	showBigPic:{
		change:'showBigPicChange',
	},
  })
  ...
  export default {
    ...
    computed: {
      datetimes:function(){
        return (datetime)=>{
          // return this.$utils.getMessageTimeFromNow(this.$utils.format(new Date(Number(datetime)),'yyyy-MM-dd hh:mm:ss'),new Date(),this.now);
    ...
    async mounted () {
      ...
      this.myScrollerHeight = document.body.clientHeight-$(".header").height()-(0.75)*fontsize-this.adjustedHeight-this.inputBarHeight;
      this.$nextTick(()=>{
	...
	if(!window.api){
      ...
      window.api.addEventListener({
        ...
        _t.sendLocationAfter(ret.value);
      });
	this.updateUserInfo();
      ...
      this.UIChatBox.open({
        ...
        styles: {
          topDivider:{                        //(可选项)顶部分割线配置
            ...
            color:'#d9d9d9'                    //(可选项)顶部分割线的颜色；默认：#000 ;支持图片设置(fs:// & widget://)
          },
          inputBar: {
            borderColor: '#f2f2f2',
            bgColor: '#f2f2f2'
      ...
      async sendGift(giftObj){
        ...
        if(giftObj){
          ...
          this.$util.sendSocket(sendObj,function(data,err,per){
            if(data){
              _t.saveMessage(1,null,null,null,null,'','[送出礼物]',data.data.msgId,null,null,extras)
            }
            if(err){
              _t.saveMessage(1,null,null,null,null,err.message,'[送出礼物]',new Date().getTime(),null,null,extras)
            }
          })
        }
      },
...
//		    return false;
      ...
      sendLocationAfter(result){
        try{
          let location={
            lon:result.lon,
            lat:result.lat,
            poi:result.name+"~"+result.address
          }
          let _t=this;
          _t.dataChats.push({
            id:new Date().getTime(),
      ...
      setMessage(type,text,imgUrl,audio,location,duration){
        ...
        if(text){
          ...
          this.$util.sendSocket(sendObj,function(data,err,per){
            if(data){
              _t.saveMessage(type,text,imgUrl,audio,location,'','[图片]',data.data.msgId,null,null)
            }
            if(err){
              _t.saveMessage(type,text,imgUrl,audio,location,err.message,'[图片]',new Date().getTime(),null,null)
            }
          })
        }else if(audio){
          let extras = {
          ...
          this.$util.sendSocket(sendObj,function(data,err,per){
            if(data){
              _t.saveMessage(type,text,imgUrl,audio,location.poi,'','[地理位置]',data.data.msgId,location.lon,location.lat)
            }
            if(err){
              _t.saveMessage(type,text,imgUrl,audio,location.poi,err.message,'[地理位置]',new Date().getTime(),location.lon,location.lat)
            }
          })
        }
      ...
      getNewMessageList(status){
        let _t = this;
        this.dataChats=this.$db.getConversation(this.targetId,this.$store.state.userId,this.limit);
