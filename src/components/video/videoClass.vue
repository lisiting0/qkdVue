<template>
  <div class="main jy_all_top">
    ...
    <div class="header">
      <div class="top">
        <Back class="left"><i class="iconfont">&#xe613;</i></Back>恋爱社交精品课程
      </div>
    </div>
    ...
    <div class="videoScroll">
        ...
        <dl class="video_panel video_panel2">
          <dt>
            <ul>
				<li :class="{cur:active==1}" @click="active!=1?getList(1):''">脱单秘籍之女生篇</li>
				<li :class="{cur:active==2}" @click="active!=2?getList(2):''">社交技巧之男生篇</li>
            </ul>
          </dt>
          <dd>
            <ul class="video_ul" v-if="listArr&&isFinishTask">
      ...
      async infinite(done) {//上拉加载(防止初始内容不满一屏会无限加载)
		...
		if(t.$route.query.taskId&&!t.isCheck){
			...
			if(t.$store.state.isLogin){
				...
				openId=t.$store.state.openId||t.$store.state.otherLogin.openId||getStore("wxOpenId");
			}
			if(openId){
				try{
