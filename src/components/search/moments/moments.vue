  ...
  <div class="main" @click="windowClick">
    ...
    <div class="header">
      <div class="top">
        ...
        <router-link :to="{name:'addMoment'}" class="right"><i class="iconfont">&#xe61f;</i></router-link>
        {{momentTitle}}
      </div>
    </div>
    <div class="title fixed" v-if="$store.state.isLogin">
      <div class="underline" ref="underline" :style="{left:underlineLeft}"></div>
      <div @click="getData(1)" :class="{active:active==1}">乾坤广场</div>
      <div @click="getData(2)" :class="{active:active==2}">乾坤动态</div>
    </div>
              ...
              <div v-for="item,index in artList" class="momentItem">
                ...
                <div>
                  <div>
                    ...
                    <p v-if="item.feedContent.length>50&&!item.openContent&&item.uploadingPerson.id==$store.state.userInfo.id" @click.stop="getDetail(index,1)">{{item.feedContent.substr(0,50)}}...</p>
                    <p v-else-if="item.feedContent.length>50&&!item.openContent" @click.stop="getDetail(index)">{{item.feedContent.substr(0,50)}}...</p>
                    <span v-if="item.feedContent.length>50&&!item.openContent" class="show_hide_text" @click="function(){item.openContent=true;}">展开</span>
                    <p v-if="item.feedContent.length<=50&&item.uploadingPerson.id==$store.state.userInfo.id||item.openContent" @click.stop="getDetail(index,1)"> {{item.feedContent}}</p>
                    <p v-else-if="item.feedContent.length<=50||item.openContent" @click.stop="getDetail(index)">{{item.feedContent}}</p>
                      ...
                      <transition v-on:before-enter="beforeEnter(item.id,1)" v-on:before-leave="beforeEnter(item.id,1)" v-on:after-enter="afterEnter(item.id,1)" v-on:after-leave="afterEnter(item.id,1)" enter-active-class="slideInRight" leave-active-class="slideOutRight">
                          <span v-show="showCommentIndex==item.id">
                            <span>
                             ...
                             <span v-else @click.stop="setFabulous(index,0)"><i class="iconfont">&#xe619;</i> 赞</span>
                             <span v-if="$store.state.userInfo.id!=item.uploadingPerson.id" @click.stop="clickShowGift(item.uploadingPerson.aliasName, item.uploadingPerson.id,item.id)"><i class="iconfont">&#xe629;</i> 打赏</span>
                  ...
                  <div class="praiseAndComment" v-if="(item.fabulous&&item.fabulous.length>0)||(item.comments&&item.comments.length>0)">
        ...
        .momentItem {
          ...
          & > div:last-child {
            ...
            .praiseAndComment {
              ...
              .jy_m_fabulous {
                font-size: 0.4rem;
                padding: 0.1rem 0.2rem;border-bottom: 1px solid #e8e7e7;
                color: #667592;
                i {
                  font-size: 0.4rem;
                }
