<template>
  <div class="shade" @click="selectPhoto">
	...
	<input v-if="($store.state.isBrowser&&!$store.state.isWeixin)||$store.state.isWindowsWechat||$store.state.isWechatdevtools" type="file" name="fileArray" class="upload__input" :id="'upload_input'+time" @change="uploadChange"  accept="image/png,image/jpeg,image/jpg">
  </div>
</template>
<script>
  ...
  export default {
    ...
    data(){
      return {
		...
		time:new Date().getTime()+""+Math.round(Math.random()*1e5),
      }
    },
	mounted(){
    ...
    methods:{
      ...
      async selectPhoto(){
		...
		if((t.$store.state.isBrowser&&!t.$store.state.isWeixin)||t.$store.state.isWindowsWechat||t.$store.state.isWechatdevtools){
			$("#upload_input"+t.time).trigger("click");
