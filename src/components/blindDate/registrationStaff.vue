<template>
  <div class="main jy_all_top jy_state_top " :class="{jy_fix_ziliao:showMb}">
    ...
    <myScroller :style="$store.state.isBrowser?'padding:3.05rem 0 0;':'padding:3.95rem 0 0;'" :infinite="infinite" :refresh="refresh" :loadRefresh="loadRefresh" :noDataText="noDataText" :loadInfinite="loadInfinite" ref="myScroller" :showbg="showbg">
          ...
          <li @click="linkTo('userInfo',item.jiaoyouUser.id,item.id,item.jiaoyouUser.silentState)" v-for="(item,index) in listArr" :key="item.id">
            <a :style="'background-image:url('+getFullPath(item.jiaoyouUser.headimgAttachmentId)+');'">
              ...
              <div class="bottom_info">
                <p><span>{{item.jiaoyouUser.aliasName}}</span><i class="jy_auth_age" :class="{jy_auth_age_man:item.candidateSex==2}">{{item.candidateSex==1?'&#xe64a;':'&#xe605;'}}{{item.jiaoyouUser.age}}</i><span v-if="$store.state.userInfo.identity!=1&&$store.state.userInfo.sex!=item.candidateSex">{{item.jiaoyouUser.matchAgree}}%</span></p>
