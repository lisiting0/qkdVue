  ...
  <div class="">
    ...
    <!--<div class="header">-->
      <!--<div class="top">-->
        ...
        <!--<p>相亲广场</p>-->
        <!--<a v-if="!$store.state.isBrowser&&$store.state.userInfo.identity==1" class="right" :class="{right2:!$store.state.isBrowser}" @click="linkTo('publishBlindDate')">发布</a>-->
        <!--<a class="right" style="top: 0.24rem" :class="{right2:!$store.state.isBrowser}" @click="linkToUserInfo('myBlindDate')" v-else-if="$store.state.isLogin">-->
    ...
    <!--<div class="fixed search_box">-->
      <!--<div class="search_input">-->
        ...
        <!--<span @click="showFilterm">筛选</span>-->
      <!--</div>-->
    <!--</div>-->
    <!--<activityFilter :showFilter="showFilter" :location="location"  @close="showFilter=false" @filterMakesure="filterMakesure" @changeLocation="changeLocation" @changePlace="changePlace"></activityFilter>-->
	<!--<transition enter-active-class="slideInUp" leave-active-class="slideOutDown">
      <setSort v-show="showFilter" @shPlacePicker="placePicker" @shStartDate="shStartDate" @shEndDate="shEndDate" :showStartDate="showStartDate" :showEndDate="showEndDate" :placePicker="showPlacePicker" @close="closePopover" @makesure="filterMakesure" :keytype="{place:true,date:true,blindType:true}" :areaColumns="2"></setSort>
    </transition>-->
	
    <myScroller :style="$store.state.isBrowser?'padding:1.73rem 0 0;':'padding:2.63rem 0 0;'" :infinite="infinite" :refresh="refresh" :loadRefresh="loadRefresh" :loadInfinite="loadInfinite" ref="myScroller" :showbg="showbg">
...
//	import setSort from '@/components/affinity/setSort'
  ...
  export default {
    name: "registrationActivityLove",//报名活动
    directives: {
      TransferDom
    },
    props:['myactivity'],
    ...
    async mounted() {
      console.log("调用了mounted方法",this.myactivity)
      const t = this;
      ...
      let shareLink =this.$store.state.baseURL+"/api/account/publicAddressLoginUrl?state=love"
      if(t.$store.state.userInfo.id){
        shareLink = shareLink + "__pid_"+this.$store.state.userInfo.jyNumber + "_mytaskId_34"
      }
    ...
    methods: {
		...
		getFee(item){
			...
			if(item.datingDetailsExt&&item.datingDetailsExt.extString6){//有渠道信息
				...
				function getChannel(ch,flag){
					if(ch){//有合伙人专属渠道
						//console.log(t.blindDateSingle.newUserFree,ch.newUserFree)
      ...
      async getList(reload,flag) {
        ...
        try {
          ...
          const result = await t.$server.blindDatingList({
            ...isRefresh,
            returnMyPublisher:t.myactivity?2:null,
            areaId: t.location == 1 ? t.selectArea&&t.selectArea[t.selectArea.length - 1] : '',
            beginActivityStarttime: t.datingTime == 1 ? t.startDate?t.startDate + " 00:00:01" :"": "",
            endActivityStarttime: t.datingTime == 1 ? t.endDate?t.endDate + " 23:59:59":"" : "",
      ...
      filterMakesure(result) {
		let {location, selectArea, selPlace, datingTime, beginActivityStarttime, endActivityStarttime, specialTypeVal, state}=result;
        if (location == 1) {
        ...
        this.startDate = beginActivityStarttime;
        this.endDate = endActivityStarttime;
