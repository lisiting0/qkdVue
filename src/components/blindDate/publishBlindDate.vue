  ...
  <div class="main jy_all_top publish_bd jy_state_top" key="main">
    ...
    <div class="userInfo_panel">
      ...
      <div class="userInfo_con">
        <div class="entry_div">
          <label>报名人数</label><input type="number" placeholder="请填写报名总人数" v-model="numberOfPeople"/>
        </div>
      </div>
      <div class="userInfo_con">
        <div class="entry_div">
          <label>报名费用</label><input type="number" placeholder="请填写报名费用" v-model="registrationFee"/>
        </div>
      </div>
      <div class="userInfo_con">
        <div class="entry_div">
          <label>男士人数</label><input type="number" placeholder="请填写男士人数" v-model="extInt8"/>
        </div>
      </div>
      <div class="userInfo_con">
        <div class="entry_div">
          <label>女士人数</label><input type="number" placeholder="请填写女士人数" v-model="extInt9"/>
        </div>
      </div>
    ...
    <div class="userInfo_panel last_panel" style="margin-top: 10px">
      <div class="userInfo_con" @click="addExtString6">
        <div>添加渠道设置</div>
        <div></div>
        <div><i class="iconfont">&#xe61f;</i></div>
      </div>
      <!--<div v-for="v,vkey in extString6" :key="vkey" class="userInfo_con" @click="showExtString6(vkey,v)">-->
        ...
        <!--<div><i class="iconfont">&#xe702;</i></div>-->
      <!--</div>-->
      <div v-for="v in extString6Sort" :key="v.key" class="userInfo_con" @click="showExtString6(v.key,v.ov)">
        <div>{{v.ov.name}}</div>
        <div>男：{{v.ov.boyLimit}}人，{{v.ov.boyFee}}元；女：{{v.ov.girlLimit}}人，{{v.ov.girlFee}}元</div>
        <div><i class="iconfont">&#xe702;</i></div>
      </div>
      ...
      <popup v-model="shOrganizerList" height="100%" :popup-style="{zIndex: 999}">
        <div class="shade" :hide-on-blur="true">
          <div class="main jy_all_top jy_state_top">
            ...
            <myScroller :style="$store.state.isBrowser?'padding:2.2rem 0 0;':'padding:3.1rem 0 0;'" :infinite="infinite" :refresh="refresh" :loadRefresh="loadRefresh" :loadInfinite="loadInfinite" ref="myScroller">
              <div class="organizers_container">
                <div class="organizers_panel">
          ...
          <div class="main jy_all_top jy_state_top">
            ...
            <div class="jy_art_content">
              <div class="setting">
                ...
                <div v-if="settingExtString6.key!='a9fDfoS'">
                  <label>合伙人分成</label><input type="number" placeholder="请填写合伙人分成" v-model="settingExtString6.share"/>
                </div>
                <div v-if="settingExtString6.key!='a9fDfoS'" class="switch_div">
                  <label>是否需要渠道认证</label><inline-x-switch :value-map="['0', '1']"  v-model="settingExtString6.needVerify"></inline-x-switch>
                </div>
				<div class="switch_div">
                  <label>新用户免费</label><inline-x-switch :value-map="['0', '1']"  v-model="settingExtString6.newUserFree"></inline-x-switch>
                </div>
              </div>
            </div>
          </div>
    ...
    <div v-transfer-dom>
      <popup v-model="shaddExtString6" height="100%" :popup-style="{zIndex: 999}">
        <div class="shade" :hide-on-blur="true">
          <div class="main jy_all_top jy_state_top">
            <div class="header">
              <div class="top">
                <a class="left" @click.stop="shaddExtString6=false"><i class="iconfont">&#xe613;</i></a><a class="right" @click.stop="makeSureAddExtString6()">确定</a><p>选择合伙人</p>
              </div>
            </div>
            <div class="jy_art_content">
              <div class="hhr_search"><input type="number" placeholder="请输入手机号查询" v-model="addSearchNumber"></div>
              <div class="hhr_div">
                <div v-for="v in hhrList" :key="v.id" @click="setAdd(v)">
                  <img :src="$utils.getFullPath(v.user.headimgAttachmentId)"><label>{{v.agentName}}</label><label>{{v.user.phoneNumber}}</label>
                </div>
              </div>
            </div>
          </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        shExtString6:false,
        shaddExtString6:false,
        extString6:{a9fDfoS:{boyFee:null,girlFee:null,boyLimit:null,girlLimit:null,name:"默认渠道",newUserFree:"0"},a9fyas8f:{share:null,needVerify:0,boyFee:null,girlFee:null,boyLimit:null,girlLimit:null,name:"合伙人共享渠道",newUserFree:"0"}},//报名渠道信息
    ...
    watch: {
      async addSearchNumber(newV){
        if(newV){
          clearTimeout(this.timer);
          this.timer=setTimeout(async ()=>{
            let result = await this.$server.getOrdAgent({
              "user.phoneNumber":newV
            });
            this.hhrList = result.data.list;
          },800);
        }
      },
      typeArr(){
    ...
    computed:{
      extString6Sort:function(){
        let obj=this.$utils.deepCopy(this.extString6);
        let arr=[];
        arr[0]={
          key:"a9fDfoS",
          ov:obj.a9fDfoS
    ...
    methods: {
      async addExtString6(){
        this.shaddExtString6=true;
        this.addSearchNumber="";
        let result = await this.$server.getOrdAgent();
        this.hhrList = result.data.list;
      },
      setAdd(obj){
        this.settingExtString6 ={share:null,needVerify:0,boyFee:null,girlFee:null,boyLimit:null,girlLimit:null,name:obj.agentName,newUserFree:0};
        this.settingExtString6.key=obj.user.jyNumber+"";
        this.shaddExtString6=false;
        this.shExtString6=true;
      },
      makeSureExtString6(){
        if(this.settingExtString6.key){
          if(this.settingExtString6.share==null&&this.settingExtString6.needVerify==0&&this.settingExtString6.newUserFree==0&&this.settingExtString6.boyFee==null&&this.settingExtString6.girlFee==null&&this.settingExtString6.boyLimit==null&&this.settingExtString6.girlLimit==null){
      ...
      async fileBack(obj) {//海报
        ...
        if (t.file) {
		  ...
		  param.append('dataSources','publishBlindDate');
