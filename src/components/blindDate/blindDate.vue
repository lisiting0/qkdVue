...
<div class="jy_act_scroll jy_act_blindPrize">
	...
	<div v-transfer-dom>
		...
		<div class="top_userInfo" v-if="showInfo">
			<userInfo :actDatingId="datingId" :actGetGoodDatingId="datingId" @openWin="openWin" @hiddenInfo="hiddenInfo" :userId="userId"></userInfo>
		</div>
		</popup>
	</div>
		...
		<div class="jy_bl_Choose">
			...
			<ul class="jy_bl_Choose_list">
				...
				<draggable v-model="chooseList" @update="dragEnd" :options = "{animation:500,ghostClass:'jy_choose_drag'}">
					<transition-group>
						<li v-for="item,index in chooseList" :key="item.id"><a :style="'background-image:url('+$utils.getFullPath(item.jiaoyouUser.headimgAttachmentId)+')'"><em @click.stop="delChoose(index)">&#xe7de;</em><span>序号:{{item.ext&&(getSex(item.candidateSex)+item.ext.extInt+"号")}}</span><p><i>{{item.jiaoyouUser.age}}岁</i>{{item.jiaoyouUser.aliasName}}</p></a></li>
					</transition-group>
				</draggable>
				</template>
				<li v-else><a></a></li>
					...
					if(t.chatRoomInfo.status==1&&!t.isMatch&&t.agreePrize!=1&&t.isSign){//没匹配的,没提交/同意奖池,已签到
						...
						if(newVal==1){//盲选
							...
							if(!t.isManage&&t.agreePrize==0&&!t.isActEnd){//非主持/没匹配的.
								t.$vux.confirm.show({
									title:"加入抽奖",
									content:t.$store.state.custom_config.blindAgreePrize||"是否加入奖池(加入后,对方可能会在抽奖中跟您匹配成功)",
									cancelText:'再想一想',
									maskZIndex:98,
									onCancel () {
				...
				(function getDatingStatus(time){
					...
					t.timeoutBlind=setTimeout(async ()=>{
						try{
							...
							if(t.$route.name!="blindDate"){//预防跳出路由后还会执行
								return false;
							}
    ...
    quitChatRoom(){
		...
		async actStatus(){//获取活动状态
			...
			try{
				...
				//t.myLove=result.data.data.beLikeList||[];//中意我的人
				//t.youLove=result.data.data.likeList||[];//我中意的人
				//t.currMatchUsers=result.data.data.allMatchUser||[];//当前已匹配的人
				//t.onlineUser=result.data.data.onlineUser||[];//在线用户
				//t.chatRoomInfo=result.data.data;//活动信息
				t.$store.commit('CHAGECHATROOMINFO',result.data.data);//更新现场活动信息	
				//t.agreePrize=result.data.data.agreePrize;//是否加入奖池
				//t.isSign=result.data.data.status2||false;//是否已经签到
				/*if(t.isSign){
					t.$vux.toast.show({
						type:"text",
						text: "您的序号为: "+t.isSign+" ,请牢记!",
				...
				/*if(result.data.data.matchOtherUser){//已经匹配成功的
					t.isMatch=true;
					t.myMathch=result.data.data.matchOtherUser;
				}*/
					...
					if (ret) {
						if(ret.eventType=="success"){
							...
							if(param.name=="sign"&&param.query.id){//签到
								...
								if(data.type=="signIn"&&data.from=='blindDate'){//相亲签到
									try{
										const result =await t.$server.blindDateSignIn({
