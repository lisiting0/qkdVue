    ...
    <template v-if="blindDateSingle">
     ...
     <div class="ybm_list" v-if="blindDateSingle.state!=5&&blindDateSingle.candidateList&&blindDateSingle.candidateList.length>0">
	  ...
	  <ul class="jy_act_staticContent" v-if="actContent" :style="actContent.styles">
		<template v-if="actContent.content&&actContent.content.pic">
		<li v-for="item,index in actContent.content.pic" :style="actContent.content.styles">
			<img v-if="item.src" class="jy_act_staticContent_img" :src="fillpath(item.src)">
			<template v-if="actContent.content.imgs&&actContent.content.imgs[item.id]">
				<img v-for="imgs in actContent.content.imgs[item.id]" class="jy_act_staticContent_imgs" :src="fillpath(imgs.src)" :style="imgs.styles">
			</template>
			<div v-if="(actContent.content.gifs&&actContent.content.gifs[item.id])||(actContent.content.routerLink&&actContent.content.routerLink[item.id])||(actContent.content.aLink&&actContent.content.aLink[item.id])" class="jy_act_staticContent_pos" :style="item.styles">
				<template v-if="actContent.content.gifs&&actContent.content.gifs[item.id]">
					<img v-for="gif in actContent.content.gifs[item.id]" class="jy_act_staticContent_gif" :style="gif.styles" :src="fillpath(gif.src)"/>
				</template>
				...
				<template v-if="actContent.content.aLink&&actContent.content.aLink[item.id]">
					<!--<template v-if="$store.state.isBrowser">
						<a class="jy_act_staticContent_link" v-for="aLink,ind in actContent.content.aLink[item.id]" :key="ind" :style="aLink.styles" :href="aLink.href" :target="aLink.target"></a>
					</template>
					<template v-else>-->
						<!--<router-link :to="{name:'outLink',query:{link:encodeURIComponent(aLink.href)}}" class="jy_act_staticContent_link" v-for="aLink,ind in actContent.content.aLink[item.id]" :key="ind" :style="aLink.styles"></router-link>-->
						<outLink :tag="'i'" :toUrl="aLink.href" class="jy_act_staticContent_link" v-for="aLink,ind in actContent.content.aLink[item.id]" :key="ind" :style="aLink.styles"></outLink>
					<!--</template>-->
				</template>
			</div>	
		</li>
		...
		<template v-else-if="isLogin">
			...
			<template  v-else>
				...
				<!-- 活动已结束 -->
				<router-link v-if="blindDateSingle.state==5" tag="em" :to="{name:'blindDateNew',query:{id:id}}" class="grey">活动结束</router-link>
				<!-- 活动没结束 -->
				<router-link tag="em" :to="{name:'blindDateNew',query:{id:id}}" class="orange">进入现场</router-link>
			</template>
		</template>
      </p>
    </template>
...
<script>
  ...
  import outLink from '@other/outLink.vue';
...
import routerBack from '@/plus/routerBack.js';
	routerBack.init("activityDetailStatic",{
		showPay:{
			fn:"hiddenPayWin"
		},
		showShare:null
  ...
  export default {
    ...
    watch: {
		async '$route' (to, from) {
			...
			let shareTitle=t.actContent.shareTitle||t.actContent.name||t.blindDateSingle.datingTitle;
			let shareDesc=t.actContent.shareDesc||t.actContent.name||t.blindDateSingle.datingTitle;
			...
			if(t.isLogin){
				...
				shareLink = shareLink + "_uid_"+this.$store.state.userInfo.id;
				if(t.actContent.taskId){
					shareLink = shareLink + "_mytaskId_"+t.actContent.taskId;
				}
			}
    ...
    methods: {
      ...
      async getBlindDateSingle() {
		...
		if(_this.actContent.countDown){
			let endTime=_this.blindDateSingle.datingDetailsExt.extDatetime;
