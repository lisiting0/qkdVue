<template>
  <div style="overflow:hidden;">
    <div v-show="pagerouter==='main'" class="page main publishAppointment" ref="scrollTop">
      ...
      <div class="header">
        <div class="top">
          <a class="left" @click.stop="showConfirmm"><i class="iconfont">&#xe613;</i></a>
      ...
      <template v-if="panel==1">
        ...
        <div class="panel marginBottom">
          ...
          <div class="check-box">
            <slot v-for="item,index in datingMethodList">
              <span class="check-method" :class="{active: item.value==datingMethod}" @click="datingMethodClick(index)">{{item.label}}</span>
            </slot>
          </div>
          <div class="tips">
            <p>{{datingMethodTitle}}说明:</p>
            <p>{{datingMethodDesc}}</p>
          </div>
        </div>
        <!--<div class="panel marginBottom">-->
          <!--<p class="title feeTitle" @click.stop="setFeePopul()">活动费用<span>{{pay==0?'AA':pay==1?'我请客':pay==3?'男A女免':'视情况而定'}}</span><i class="iconfont">&#xe702;</i></p>-->
          <!--&lt;!&ndash;<div class="check-box">&ndash;&gt;-->
            <!--&lt;!&ndash;<span class="fee" @click="fee=0" :class="{active:fee==0}">免费</span>&ndash;&gt;-->
            <!--&lt;!&ndash;<span class="fee" @click="fee=100" :class="{active:fee==100}"><i class="qkb-icon"></i>100</span>&ndash;&gt;-->
            <!--&lt;!&ndash;<span class="fee" @click="fee=200" :class="{active:fee==200}"><i class="qkb-icon"></i>200</span>&ndash;&gt;-->
            <!--&lt;!&ndash;<span class="fee" @click="fee=500" :class="{active:fee==500}"><i class="qkb-icon"></i>500</span>&ndash;&gt;-->
          <!--&lt;!&ndash;</div>&ndash;&gt;-->
        <!--</div>-->
        <!--<div class="panel marginBottom">-->
      ...
      <template v-if="panel==2">
        <div class="first-panel marginBottom02 select_panel">
          <template v-if="datingThemes==2||datingThemes==4||datingThemes==3"  ><!-- 吃饭 唱歌 电影-->
            <div v-if="datingThemes==3" class="flex">
              <div>电影名称</div><div><input class="datingTitle" v-model="datingTitle" placeholder="请输入影片名称"/></div>
            </div>
            <div class="select_con" @click="shAddress=true">
              <div>{{datingThemes==3?'观影地点':'约会地点'}}</div>
            ...
            <div class="select_con" @click="shDatingTime=true">
              <div>{{datingThemes==3?'观影时间':'约会时间'}}</div>
              <div>{{datingTimeText==""?"选择时间": datingTimeText+':00'}}</div>
              <div><i class="iconfont">&#xe702;</i></div>
          ...
          <template v-else-if="datingThemes==1"><!-- 旅行 -->
            <div class="select_con" @click="shAddress=true">
              <div>目的地</div>
              <div>{{addressText==""?"选择地点": addressText}}</div>
              <div><i class="iconfont">&#xe702;</i></div>
            ...
            <div class="select_con" @click="shDatingTime=true">
              <div>出发时间</div>
              <div>{{datingTimeText==""?"出发时间": datingTimeText+':00'}}</div>
              <div><i class="iconfont">&#xe702;</i></div>
              <group style="display: none">
              <datetime title="约会时间" v-model="datingTimeText" :show.sync="shDatingTime" :start-date="startDate" :end-date="endDate" format="YYYY-MM-DD HH" year-row="{value}年" month-row="{value}月" day-row="{value}日" hour-row="{value}时" confirm-text="完成" cancel-text="取消"></datetime>
              </group>
          ...
          <template v-else-if="datingThemes==5||datingThemes==99"><!-- 运动和其他 -->
          <div class="flex" @click="selectDatingTitle">
            <div>{{datingThemes==99?'主题':'运动项目'}}</div><div><span>{{datingTitle===''?datingThemes==99?'选择主题':'选择运动项目':datingTitle}}</span><i class="iconfont">&#xe702;</i></div>
          </div>
          <div class="select_con" @click="shAddress=true">
            <div>约会地点</div>
            <div>{{addressText==""?"选择地点": addressText}}</div>
          ...
          <div class="select_con" @click="shObjTime=true">
            ...
            <div><i class="iconfont">&#xe702;</i></div>
            <group style="display: none">
              <datetime title="约会对象时间" v-model="objTimeText" :show.sync="shObjTime" :start-date="startDate" :end-date="endDate" format="YYYY-MM-DD HH:mm" year-row="{value}年" month-row="{value}月" day-row="{value}日" hour-row="{value}时" minute-row="{value}分" :minute-list='minuteList' confirm-text="完成" cancel-text="取消"></datetime>
            </group>
          </div>
        ...
        <div class="marginBottom02 select_panel">
          ...
          <!--<div class="select_con" v-if="!$store.state.isiOSApp">-->
            <!--<div>买单</div>-->
            <!--<div class="row-check-box">-->
              <!--<span class="box" @click="pay=0" :class="{active:pay==0}">AA</span>-->
              <!--<span class="box" @click="pay=1" :class="{active:pay==1}">我请客</span>-->
        ...
        <!--<div class="panel"  v-if="!$store.state.isiOSApp&&!$store.state.isBrowser">-->
          <!--<p class="title">见面礼<i class="iconfont" style="color: #ff4200;">&#xe682;</i></p>-->
          <!--<div class="right-con">-->
            <!--<div class="left-title">我要送</div>-->
            <!--<template v-if="expGift==0">-->
              <!--<shop :tag="'div'" :toUrl="$store.state.baseShopMobileURL+'/category.php?id=221&isGift=1&isGive=1'" class="right-title"><p>去选礼物</p></shop>-->
              <!--<div class="next-icon"><i class="iconfont" style="color: #f2f2f2;">&#xe702;</i></div>-->
            <!--</template>-->
            <!--<template v-else>-->
              <!--<div class="right-title">-->
                <!--<div class="gift-info">-->
                  <!--<div class="gift-icon" :style="'background-image:url('+$store.state.phpURL + '/'+ giveItems.imagePath+');'"></div>-->
                  <!--<p>{{giveItems.name}}* {{giveItems.num}}</p>-->
                  <!--<p>价值：{{giveItems.price}}</p>-->
                <!--</div>-->
              <!--</div>-->
              <!--<div class="next-icon" @click.stop="delGiveGift"><i class="iconfont" style="color: #ff4200;">&#xe67c;</i></div>-->
            <!--</template>-->
          <!--</div>-->
          <!--<div class="right-con">-->
            <!--<div class="left-title">我要收</div>-->
            <!--<template v-if="sendGift==0">-->
              <!--<shop :tag="'div'" :toUrl="$store.state.baseShopMobileURL+'/category.php?id=221&isGift=1&isGive=2'" class="right-title"><p>去选礼物</p></shop>-->
              <!--<div class="next-icon"><i class="iconfont" style="color: #f2f2f2;">&#xe702;</i></div>-->
            <!--</template>-->
            <!--<template v-else>-->
              <!--<div class="right-title">-->
                <!--<div class="gift-info">-->
                  <!--<div class="gift-icon" :style="'background-image:url('+ $store.state.phpURL + '/'+ sendItems.imagePath+');'"></div>-->
                  <!--<p>{{sendItems.name}}* {{sendItems.num}}</p>-->
                  <!--<p>价值：{{sendItems.price}}</p>-->
                <!--</div>-->
              <!--</div>-->
              <!--<div class="next-icon" @click.stop="delSendGift"><i class="iconfont" style="color: #ff4200;">&#xe67c;</i></div>-->
            <!--</template>-->
          <!--</div>-->
        <!--</div>-->
        <div class="panel marginBottom">
    ...
    <div class="main selectdatingTitle" v-show="pagerouter==='datingTitle'">
	
      <div class="header">
        <div class="top">
          <a class="left" @click.stop="confirm111(1)"><i class="iconfont">&#xe613;</i></a><a @click.stop="confirm111()" class="right"><i class="iconfont">确定</i></a>邀约主题
        </div>
      </div>
	 
      <div :style="$store.state.isBrowser?'margin-top: 1.39rem;':'margin-top: 2.14rem;'" class="wrapper" style="overflow: hidden">
        <div class="content" style="font-size: 0.4rem;">
          <div class="white" style="margin: 0.3rem 0 0;padding: 0 0.4rem;">
            请勿发布商品广告及色情招嫖等违法信息，否则将永久封号处理
          </div>
          <div class="white" style="margin: 0.3rem 0 0">
            <div class="placediv">
              <input class="placeinput" v-model="datingTitleCache" type="text" placeholder="请选择或填写一个主题"/>

            </div>
          </div>
          <p class="datingTitleDetail" style="margin: 0.4rem 0 0 0.4rem;">
            <span @click="datingTitleCache=item.value" v-for="item in datingTitleOtherType" :class="{active:item.value==datingTitleCache}">{{item.value}}</span>
          </p>
        </div>
        <!--<group style="display: none">-->
...
<script>
  ...
  routerBack.init("publishAppointment",{
    showMapPopup:{
      fn:"hiddenMap",
    },
    showStorePopup:{
  ...
  export default {
	  ...
	  showPage:{
		handler(newV, oldV) {
			...
			if(!newV){
				...
				this.pagerouter='datingTitle';
			}
			this.$store.commit("CHANGEHANDLEROUTE", true);//特殊 默认是true
		},
	  }
    },
    computed: {
    ...
    methods: {
      showConfirmm(){
		...
		//console.log("222222222222222"+t.showConfirm)
      ...
      selectDatingTitle(){
        this.datingTitleCache = this.datingTitle
        this.pagerouter='datingTitle';
      },
      goback() {
        this.$router.go(-1)
        ...
        if(obj.serverId){
          for(let i=0;i<obj.serverId.length;i++){
            let result = await t.$server.wxMpMedia({
			  ...
			  dataSources:'publishAppointment'
            })
          ...
          if (t.fileList) {
            for (let i = t.fileList.length; i--;) {
			  ...
			  param.append('dataSources','publishAppointment');
              try {
      ...
      async finish(state){
        ...
        if(_this.panel==2){
          if(_this.datingDesc==''){
			...
			$("body,html").animate({scrollTop:0},0);
            setTimeout(()=>{
				this.$refs["datingDesc_input"]&&this.$refs["datingDesc_input"].focus();
			},50)
            return;
          }
        }
        if(_this.panel==3){
          ...
          _this.giftDetail = [];
          if(_this.expGift==1){
            _this.giftDetail.push(_this.giveItems);
          }
          if(_this.sendGift==1){
            _this.giftDetail.push(_this.sendItems);
          }
        }
        let data =  {
          id: _this.id,
          ...
          datingStarttime: _this.datingTimeText?_this.datingTimeText+':00:00':'',
