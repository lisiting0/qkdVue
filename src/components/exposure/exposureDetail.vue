            ...
            <!--<template v-if="moment">-->
              <template v-if="dynamic">
                <div class="momentItem">
                  <div><span  class="headImg" :style="{'background-image':'url('+$utils.getFullPath(dynamic.anonymous==1?'':dynamic.user.headimgAttachmentId)+')'}"></span></div>
                  <div>
                    <div>
                      <p>{{dynamic.anonymous==1?'匿名':dynamic.user.aliasName||'匿名'}}</p>
					  <p v-if="dynamic.lightUser" class="jy_exp_user">被曝光人：{{dynamic.lightUser.aliasName}}（乾坤号：{{dynamic.lightUser.jyNumber}}）</p>
                      <p v-if="dynamic.descript.length>50&&!dynamic.openContent">
                        {{dynamic.descript.substr(0,50)}}...
                        <span @click="function(){dynamic.openContent=true;}">展开</span>
                      </p>
                      <p v-if="dynamic.descript.length<=50||dynamic.openContent">
                        {{dynamic.descript}}
                        <span v-show="dynamic.descript.length>50" @click="function(){dynamic.openContent=false; }">收起</span>
                      </p>
                    </div>
                    <div @click="showImage(0,dynamic.id,$event)" v-if="dynamic.picture&&dynamic.picture.split(',').length==1" class="one-cls" :class="'previewer-'+dynamic.id">
                      <div><img :src="getFullPath(dynamic.picture.split(',')[0])"/>
                      </div>
                    </div>
                    <div v-else-if="dynamic.picture" class="img-cls" :class="'previewer-'+dynamic.id">
                      <div v-for="imgItem ,imgIndex in dynamic.picture.split(',')" @click="showImage(imgIndex,dynamic.id,$event)" :style="{'background-image':'url('+getFullPath(imgItem)+')'}">
                    ...
                    <div v-transfer-dom>
                      <previewer :list="getImage(dynamic.picture.split(','))" :ref="'previewer'+dynamic.id" :options="options" @on-close="closeImg"></previewer>
                    </div>
                    <!--<p v-if="dynamic.location">定位</p>-->
                    <div class="publish-cls">
                      <!--<span v-if="dynamic.longitude">定位</span>--><span>{{getMessageTimeFromNow(dynamic.createDate)||'刚刚'}}</span><!--<span v-if="dynamic.user.id==$store.state.userInfo.id" style="margin-left: 0.2rem;" @click="delDynamic(dynamic.id)">删除</span>-->
                      <div :ref="dynamic.id">
