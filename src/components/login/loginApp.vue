<template>
  <div style="height: 100%;">
    <div class="login">
      ...
      <!--<div style="padding: 0.2rem 0;color: #ff788c;" @click="back"><i class="iconfont">&#xe613;</i></div>-->
      ...
      <div class="div_input">
        ...
        <input class="userName" v-model="loginName" type="text" placeholder="请输入手机号">
      </div>
      ...
      <div v-else class="div_input">
		<i class="n_login_ico">&#xe626;</i>
        <input class="passWord" v-model="password" type="password" placeholder="请输入密码" v-if="!showPassword">
        <input class="passWord" v-model="password" type="text" placeholder="请输入密码" v-else>
      ...
      <div @click="login" class="btn_login" :class="{'btn_logined':showOpacity}">
        {{$store.state.otherLogin.isBind?'登录':loginType=='code'? qkId && qkName?'加入乾坤岛':'登录':qkId && qkName?'加入乾坤岛':'登录'}}
      </div>
	...
	<p class="jy_login_nb">
		<a v-if="loginType=='password'" @click="loginType='code';password=''">手机登录</a>
		<a v-else @click="loginType='password';verificationCode=''">密码登录</a>
		<router-link :to="{name:'login',query:$route.query}" v-if="$store.state.isWeixin" class="reg">注册</router-link>
       <!-- <a v-else class="reg" href="javascript:;" @click="toRegistered">注册</a>-->
		<router-link v-if="loginType=='password'" :to="{name:'forgetPassword'}">忘记密码</router-link>
		<Return class="jy_login_nbg" v-else>随便逛逛</Return>
	</p>
      <div class="other_login">
        <i v-if="!$store.state.isBrowser&&!$store.state.isiOSApp">快捷登录</i>
        <otherLogin v-if="!$store.state.isBrowser" @changeLoading="changeLoading"></otherLogin>
        <p>{{$store.state.otherLogin.isBind?"绑定":"登录"}}代表您同意<span @click="showAgreement=true">&lt;&lt;乾坤岛用户协议&gt;&gt;</span></p>
      </div>
...
import Return from '@other/return.vue';
  ...
  export default {
   ...
   async mounted() {
	 const t=this;
		t.loginType=t.$route.query.type||"code";
      ...
      async login() {
        ...
        setTimeout(() => {
          if (t.$store.state.otherLogin.isBind) {
            if (t.$store.state.gotoBlindDatepay) {
            ...
            } else if (t.$route.query.redirectUrl || (t.$store.state.otherLogin.params&&t.$store.state.otherLogin.params.query && t.$store.state.otherLogin.params.query.redirectUrl)) {
              ...
              if (t.$store.state.otherLogin.params.name == 'login'||t.$store.state.notBack) {
                t.$router.replace({
                  path: '/home'
                })
              } else {
