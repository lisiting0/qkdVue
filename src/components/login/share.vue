    ...
    methods: {
      shareWebpage(Vscene){//分享到微信
        ...
        let obj={
          ...
          contentUrl:t.$store.state.baseURL+"/api/account/publicAddressLoginUrl?&state=shareEnter__pid_"+this.$store.state.userInfo.jyNumber+"_uid_"+this.$store.state.userInfo.id,
      ...
      shareNews(type){//分享到QQ
        ...
        let obj={
          title : '介绍大家一个纯粹、高效、安全的单身男女交友平台',
          description : '乾坤岛架设在区块链和人工智能技术之上，可以为真正有交友需求的异性男女提供一个纯粹、高效、安全的社交网络平台，它由相亲牵线、高效约会及月老宝阁三大模块组成。',
          url:t.$store.state.baseURL+"/api/account/publicAddressLoginUrl?&state=shareEnter__pid_"+this.$store.state.userInfo.jyNumber+"_uid_"+this.$store.state.userInfo.id,
