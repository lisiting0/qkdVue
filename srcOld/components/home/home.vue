  ...
  <div class="main">
    ...
    <div class="myscroll">
      <scroll ref="scroll" :pullDownRefresh="pullDownRefreshObj" :pullUpLoad="pullUpLoadObj" @pullingDown="onPullingDown" @pullingUp="onPullingUp" :ifinit="ifinit" :style="{height:myScrollerHeight+'px'}" :probeType="3" :listenScroll="true" @scroll="myscroll" :click="true">
        <div class="block">
          <div class="tit">
            <router-link :to="{name:'nearbydate',query:{nearbydate:1}}" class="more">查看更多<i class="iconfont">&#xe702;</i> </router-link>
            <span>附近约会</span>
          </div>
          <div class="content">
...
<script>
  ...
  export default {
    ...
    mounted() {
      if(window.api&&this.$store.state.isLogin){
        //this.initRongYun();
    ...
    /*  async initRongYun(){
      ...
      async getList(module,search,PullingDown,sex,authentication,areaId,datingTime,startDate,endDate){
        ...
        try{
          ...
          let result1 = await _this.$server.searchDatingList(data);
          let result2 = await _this.$server.nearUser(data);
          let areaId = null
          ...
          let result3 = await _this.$server.blindDatingList({
            areaId:areaId,
            page:1,
            rows:8
          });
         // console.log(JSON.stringify(result3))
          if(result1.data.list){
            _this.nearArr=[]
            _this.nearArr.push(...result1.data.list);
            _this.nearArr=_this.nearArr.filter((v,i)=>{
              return i<4
            })
          }
          if(result3.data.list){
            _this.registrationActivity=[]
            _this.registrationActivity.push(...result3.data.list);
            _this.registrationActivity=_this.registrationActivity.filter((v,i)=>{
              return i<4
            })
          }
          if(result2.data.list){
            _this.nearPeopleArr=[]
            _this.nearPeopleArr.push(...result2.data.list);
            _this.nearPeopleArr=_this.nearPeopleArr.filter((v,i)=>{
              return i<8
            })
          }
