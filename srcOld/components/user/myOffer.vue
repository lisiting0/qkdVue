                  ...
                  <ul class="myOffer_list">
                    <li v-for="(item,index) in listArr" :key="item.id" @click.stop="toActiveDetail(item.id)">
                      <swipeout-item transition-mode="follow">
                        ...
                        <a slot="content">
                          <i class="user_image" :style="'background-image:url('+getFullPath(item.coverimgImages?item.coverimgImages:item.jiaoyouUser.headimgAttachmentId)+');'" @click="toActiveDetail(item.id)">
                          ...
                          <div class="offer_desc">
                            ...
                            <template v-if="active==3">
                              <template v-if="item.state!=5">
                                <span class="zdyy" v-if="item.state==2" @click.stop="edit(item.id)">编辑</span>
