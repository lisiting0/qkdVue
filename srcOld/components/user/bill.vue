<template>
	<div class="main jy_all_top jy_bill">
						...
						<template v-for="item,index in transObj">
							...
							<template v-else-if="tabIndex==1">
								<li v-for="val in item.list">
									...
									<h4>{{val.channel==0?'打赏':val.channel==1?'提现':val.channel==2?'商城礼物折现':val.channel==3?'代理奖励':val.channel==4?'活动报名':'其他'}}</h4>
									<p>{{val.createDate}}</p>
		...
		<div v-transfer-dom>
			<popup v-model="showSel" position="top" :popup-style="{zIndex:596,paddingTop:$store.state.isBrowser?'4.03rem':'4.78rem'}" :should-scroll-top-on-show="true">
