		...
		<div class="jy_act_mid">
			<div class="jy_act_mid_con">
				...
				<dl class="jy_act_lt">
					...
					<dd v-if="chatList">
						<myScroller  :refreshText="refreshText" :infinite="infinite" :refresh="refresh"  :loadRefresh="loadRefresh" :loadInfinite="loadInfinite" ref="myScroller">
  ...
  export default {
    ...
    watch:{
      ...
      doMessage(newV,oldV){
		...
		showWin(val){
			const t=this;
			t.$store.commit("CHANGEHANDLEROUTE", val);
			if(val){
				t.closeKeyBoard();
			}
		},
			...
			if(t.$store.state.userInfo.idStatus==1&&t.$store.state.userInfo.silentState==0){//实名认证,关闭'不允许别人看我'
				...
				if(t.$store.state.userInfo.silentState!=0&&t.$store.state.userInfo.idStatus!=1){
					t.$vux.confirm.show({
						...
						theme: 'android',
						hideOnBlur:true,
						content: '只有关闭"不允许别人看我"以及通过"实名认证"之后，才能查看其他嘉宾的详细资料。',
						confirmText: '去实名认证',
						cancelText:'去关闭',
						onCancel () {
					...
					t.$vux.confirm.show({
						...
						theme: 'android',
						hideOnBlur:true,
						content: '只有关闭"不允许别人看我"之后，才能查看其他嘉宾的详细资料。',
						confirmText: '去关闭',
						onCancel () {
					...
					t.$vux.confirm.show({
						...
						theme: 'android',
						hideOnBlur:true,
						content: '只有通过"实名认证"之后，才能查看其他嘉宾的详细资料。',
						confirmText: '去实名认证',
						onCancel () {
