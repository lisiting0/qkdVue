<template>
  <div style="height: 100%;overflow-y: auto" ref="scroll" @scroll="scroll($event)">
        <div class="login">
		  ...
		  <p class="join_text" v-if="qkId && qkName">您的好友{{qkName}}邀请你加入乾坤岛</p>

          <selectPhoto :cutting="true" @fileBack="fileBack" :class="{cur:1|| qkId && qkName}">
          ...
          <div @click="login" class="btn_login" :class="{'btn_logined':showOpacity}">{{$store.state.gotoBlindDatepay=='baoming'?'下一步':qkId && qkName?'加入乾坤岛':'确定'}}</div>
        </div>
