...
let SlotMachines=function(elemId,opt){
	var t=this;
	t.options={
		x:4,//横向的个数(九宫格)
		y:3,//竖向的个数(九宫格)
...
SlotMachines.prototype={
	constructor:SlotMachines,
	init:function(){
		var t=this;
		t.playing=false;
		t.effect="goon";
			...
			for(var i=0;i<obj.length;i++){
				...
				if(obj[i].children.length<t.options.lap){
					for(var j=0;j<t.options.lap;j++){
						var clone=obj[i].querySelector(".luck_ani").cloneNode(true);
						console.log(clone)
				...
				if(typeof pos =="object"&&pos.length==t.length){
					...
					for(var i=0;i<t.length;i++){
						(function(i){
							...
							console.log("translateY"+childrenheight);
