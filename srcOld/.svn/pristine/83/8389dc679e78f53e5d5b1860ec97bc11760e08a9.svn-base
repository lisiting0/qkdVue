<template>
  <div style="overflow:hidden;">
    <div v-show="pagerouter==='main'" class="page main publishAppointment" ref="scrollTop">
      ...
      <template v-if="panel==2">
        <div class="first-panel marginBottom02 select_panel">
          <template v-if="datingThemes==2||datingThemes==4||datingThemes==3"><!-- 吃饭 唱歌 电影-->
            ...
            <div class="select_con" @click="shDatingTime=true">
              ...
              <datetime title="约会时间" v-model="datingTimeText" :show.sync="shDatingTime" :start-date="startDate" :end-date="endDate" format="YYYY-MM-DD HH" year-row="{value}年" month-row="{value}月" day-row="{value}日" confirm-text="完成" cancel-text="取消"></datetime>
              </group>
            </div>
          </template>
        ...
        <div class="marginBottom02 img-panel">
          ...
          <textarea v-model="datingDesc" cols="40" rows="2" placeholder="写点什么会有更多关注哦" style="border: none;margin: 0.4rem 0 0;color: #a3a3a4;font-size: 0.4rem;" @focus="setFocus" @blur="setBlur"></textarea>
        </div>
    ...
    <div v-transfer-dom>
      <popup v-model="showMapPopup" height="100%" :hide-on-blur="false" position="bottom" :popup-style="{zIndex:596}" :should-scroll-top-on-show="true">
        <div class="top_userInfo" v-if="showbMap">
          <bMap @address="setAdress" @hidden="hiddenMap"></bMap>
        </div>
      </popup>
    </div>
    <div v-transfer-dom>
      <popup v-model="showStorePopup" height="100%" :hide-on-blur="false" position="bottom" :popup-style="{zIndex:596}" :should-scroll-top-on-show="true">
        <div class="top_userInfo" v-if="showStore">
          <store :datingThemes="datingThemes" @selStore="selStore" @hidden="hiddenStore"></store>
        </div>
      </popup>
    </div>
    <div class="main selectdatingTitle" v-show="pagerouter==='datingTitle'">
      <div class="header">
        <div class="top">
          <a class="left" @click="confirm111(1)"><i class="iconfont">&#xe613;</i></a><a @click="confirm111()" class="right"><i class="iconfont">确定</i></a>邀约主题
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    watch: {
      ...
      pagerouter(newV){
        this.$store.commit("CHANGEHANDLEROUTE",newV=='datingTitle')
      },
    },
    computed: {
      startDate: function () {
    ...
    methods: {
      ...
      async getData() {
        ...
        _this.expShowPic = detail.expShowPic;
